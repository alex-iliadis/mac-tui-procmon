...
Host-wide security auditing is now exposed as the SecAuditor browser/API
product in the ``secauditor`` package. The legacy audit functions remain here
as the shared implementation layer.
"""
...
_HTML_REPORT_DIR = os.path.join(HOME, ".secauditor", "reports")
MAX_TREE_DEPTH = 20
MAX_PIDS = 65536
SORT_MEM = "m"
SORT_CPU = "c"
...
def _blocking_missing_tools(missing):
    ...
    secprocmon now always starts the core monitor. Missing integrations and
...
def _check_external_tools(scope="all"):
    ...
    sudo-run secprocmon doesn't falsely report user-installed CLIs as missing
...
def _render_preflight_report(missing, stream=None):
    ...
    print("", file=stream)
    print("secprocmon will continue in DEGRADED mode.", file=stream)
...
def _delete_tcc_grant(client, service, db_path, logger=None):
    ...
    if short_name and shutil.which("tccutil"):
        ...
        #   1. Current env — when run via `sudo mac-tui-procmon` with HOME
...
def _effective_home():
    ...
    When secprocmon is launched with `sudo mac-tui-procmon`, $HOME resolves to /var/root
...
_CHAT_TIMEOUT_SECS = max(
    60, int(os.environ.get("SECPROCMON_CHAT_TIMEOUT", "300"))
)
...
def _santa_status_snapshot(timeout=10):
    """Run `santactl status` (prefer JSON when supported)."""
    rc, out, err = _run_external_tool("santa", ["status", "--json"], timeout=timeout)
    if rc is None:
        return {"error": err or "santa unavailable", "raw": "", "mode": "", "running": False}
    text = (out or err or "").strip()
    if rc == 0 and text.startswith(("{", "[")):
        try:
            data = _json.loads(text)
        except ValueError:
            data = None
        if isinstance(data, dict):
            flat = _json.dumps(data).lower()
            mode_match = re.search(r'"mode"\s*:\s*"([A-Z_]+)"', text)
            return {
                "error": "",
                "raw": text,
                "mode": (mode_match.group(1) if mode_match else ""),
                "running": "not running" not in flat and "error" not in flat,
                "json": data,
            }
    if rc != 0:
        # Fall back to plain-text status before giving up.
        rc2, out2, err2 = _run_external_tool("santa", ["status"], timeout=timeout)
        if rc2 is None:
            return {"error": err2 or err or "santa unavailable", "raw": "", "mode": "", "running": False}
        text = (out2 or err2 or "").strip()
        rc = rc2
    lower = text.lower()
    mode_match = re.search(r"\b(MONITOR|LOCKDOWN|UNKNOWN)\b", text)
    return {
        "error": "" if rc == 0 else text[:240] or f"exit code {rc}",
        "raw": text,
        "mode": mode_match.group(1) if mode_match else "",
        "running": "not running" not in lower and "error occurred communicating" not in lower,
    }


def _santa_doctor_snapshot(timeout=20):
    """Run `santactl doctor` and collect issue-like lines."""
    rc, out, err = _run_external_tool("santa", ["doctor"], timeout=timeout)
    if rc is None:
        return {"error": err or "santa unavailable", "issues": [], "raw": ""}
    text = (out or err or "").strip()
    if rc != 0 and not text:
        text = f"exit code {rc}"
    return {
        "error": "" if rc == 0 else text[:240],
        "issues": _summarize_issue_lines(text, limit=10),
        "raw": text,
    }


_SANTA_CONFIG_DOMAINS = (
    "com.northpolesec.santa",
    "com.google.santa",
)
_SANTA_DEFAULT_EVENT_LOG_PATH = "/var/db/santa/santa.log"
_SANTA_INTERESTING_EVENTS = {
    "Execution",
    "Authentication",
    "OpenSSH",
    "LoginWindowSession",
...
def _santa_config_snapshot():
    """Return the active Santa configuration domain and log settings."""
    for domain in _SANTA_CONFIG_DOMAINS:
        data = _read_plist_defaults(domain)
        if not data:
            continue
        telemetry = data.get("Telemetry") or []
        if isinstance(telemetry, str):
            telemetry = [telemetry]
        elif not isinstance(telemetry, list):
            telemetry = []
        prefix_filters = data.get("FileChangesPrefixFilters") or []
        if isinstance(prefix_filters, str):
            prefix_filters = [prefix_filters]
        elif not isinstance(prefix_filters, list):
            prefix_filters = []
        return {
            "domain": domain,
            "event_log_type": str(data.get("EventLogType") or "file"),
            "event_log_path": str(
                data.get("EventLogPath") or _SANTA_DEFAULT_EVENT_LOG_PATH),
            "telemetry": [str(item) for item in telemetry if item],
            "file_changes_regex": str(data.get("FileChangesRegex") or ""),
            "file_changes_prefix_filters": [
                str(item) for item in prefix_filters if item
            ],
            "machine_id_decoration": bool(
                data.get("EnableMachineIDDecoration", False)),
        }
    return {
        "domain": "",
        "event_log_type": "file",
        "event_log_path": _SANTA_DEFAULT_EVENT_LOG_PATH,
        "telemetry": [],
        "file_changes_regex": "",
        "file_changes_prefix_filters": [],
        "machine_id_decoration": False,
    }


def _santa_flatten_event(node, prefix="", out=None):
    """Flatten a Santa telemetry JSON object into string-keyed scalars."""
    if out is None:
        out = {}
    if isinstance(node, dict):
        for key, value in node.items():
            ...
            if isinstance(value, dict):
                _santa_flatten_event(value, next_prefix, out)
                ...
                if scalars:
                    out[next_prefix.lower()] = ", ".join(scalars)
            elif value not in (None, ""):
                out[next_prefix.lower()] = str(value)
    return out


def _parse_santa_telemetry_line(line):
    """Parse one Santa telemetry log line from JSON or key=value format."""
    text = (line or "").strip()
    if not text:
        return None
    flat = {}
    if text.startswith(("{", "[")):
        try:
            ...
            data = None
        if isinstance(data, dict):
            flat = _santa_flatten_event(data)
    if not flat:
    ...
    kind_key = re.sub(r"[^a-z0-9]", "", kind_raw.lower())
    kind = {
        "execution": "Execution",
        "exec": "Execution",
        ...
        "fork": "Fork",
        "exit": "Exit",
    }.get(kind_key, kind_raw or "Unknown")
    decision = (
        _first("decision", "policy_decision", "result", "mode")
        .upper()
        .replace(" ", "_")
    )
    path = _first(
    ...
    if not path:
        path = _first("process", "target", "bundle_name")
    return {
        "kind": kind,
        "decision": decision,
        "path": path,
        "team_id": _first("teamid", "team_id"),
        "signing_id": _first("signingid", "signing_id"),
        "user": _first("user", "username"),
        "remote": _first("remote_ip", "ip", "remote_address", "hostname"),
        "sha256": _first("sha256", "file_sha256"),
        "time": _first("time", "timestamp", "event_time"),
        "raw": text[:240],
    }


def _read_santa_telemetry(limit=60):
    """Summarize recent Santa telemetry from its configured on-disk log."""
    cfg = _santa_config_snapshot()
    log_type = (cfg.get("event_log_type") or "file").lower()
    path = cfg.get("event_log_path") or _SANTA_DEFAULT_EVENT_LOG_PATH
    telemetry = cfg.get("telemetry") or []
    if log_type in ("syslog", "protobuf", "null"):
        return {
            "error": f"EventLogType={log_type} is not readable from disk",
            "path": path,
            "log_type": log_type,
            "telemetry": telemetry,
            "counts": {},
    ...
    recent = [line for line in raw_lines[-max(limit * 20, 400):] if line.strip()]
    events = []
    counts = {}
    decisions = {}
    for line in recent:
        evt = _parse_santa_telemetry_line(line)
        if not evt:
            continue
        events.append(evt)
        counts[evt["kind"]] = counts.get(evt["kind"], 0) + 1
        if evt["decision"]:
            decisions[evt["decision"]] = decisions.get(evt["decision"], 0) + 1
    ...
    for evt in events[::-1]:
        if len(samples) >= limit:
            break
        interesting = (
            evt["kind"] in _SANTA_INTERESTING_EVENTS
            or evt["decision"] in ("DENY", "DENIED", "BLOCK", "BLOCKED", "UNKNOWN")
        )
        if not interesting:
            continue
        sample_bits = [evt["kind"]]
        if evt["decision"]:
            sample_bits.append(evt["decision"])
        if evt["path"]:
            sample_bits.append(evt["path"])
        elif evt["user"]:
            sample_bits.append(f"user={evt['user']}")
        if evt["remote"]:
            sample_bits.append(f"remote={evt['remote']}")
        samples.append(" ".join(sample_bits)[:180])
...
def _sfltool_dumpbtm():
    ...
    try:
        if os.geteuid() != 0:
            # On recent macOS builds this can trigger an Authorization prompt
            # from inside a scan. SecAuditor must never ask for credentials
            # mid-run; root-launched scans collect this backend.
            return ""
    except AttributeError:
        return ""
...
def _audit_persistence():
    ...
    if kk.get("error"):
        findings.append({
            "severity": "INFO",
            "message": f"KnockKnock scan unavailable: {kk['error']}",
            "action": None,
        })
    else:
        flagged = int(kk.get("flagged_items", 0))
        total = int(kk.get("persistent_items", 0))
        if flagged > 0:
            findings.append({
                "severity": "HIGH",
                "message": f"KnockKnock flagged {flagged} persistent item(s)",
                "evidence": "\n".join((kk.get("categories") or [])[:8]),
                "action": None,
            })
        else:
            findings.append({
                "severity": "INFO" if total else "OK",
                "message": f"KnockKnock enumerated {total} persistent item(s)",
                "evidence": "\n".join((kk.get("categories") or [])[:8]),
                "action": None,
            })

    # ---- Objective-See BlockBlock log telemetry ----
    bb = _read_blockblock_summary(limit=10)
    if bb.get("error"):
        findings.append({
            "severity": "INFO",
            "message": f"BlockBlock telemetry unavailable: {bb['error']}",
            "action": None,
        })
    elif bb.get("blocked") or bb.get("alerts"):
        findings.append({
            "severity": "MEDIUM" if bb.get("blocked") else "INFO",
            "message": (
                f"BlockBlock recent telemetry: {bb.get('alerts', 0)} alert(s), "
                f"{bb.get('blocked', 0)} blocked action(s)"
            ),
            "evidence": "\n".join((bb.get("samples") or [])[:8]),
            "action": None,
        })
    else:
        findings.append({
            "severity": "OK",
            "message": "BlockBlock log present with no recent persistence alerts",
            "action": None,
        })
...
def _audit_installed_software():
    ...
    for app in apps:
        ...
        sig_ok = cs.get("rc") == 0
        team = cs.get("team_id", "")
        trust = _binary_trust_profile(app, cs, {})
...
def _audit_injection_antidebug():
    """Runtime injection / anti-debug audit backed by live process inspection."""
    findings = []
    osquery_rows, osquery_err = _osquery_process_snapshot(timeout=20)
    if osquery_err:
        findings.append({
            "severity": "INFO",
            "message": f"osquery telemetry backend unavailable: {osquery_err}",
            "action": None,
        })
    elif osquery_rows:
        findings.append({
            "severity": "INFO",
            "message": (
                f"osquery telemetry backend enumerated "
                f"{len(osquery_rows)} process row(s)"
            ),
            "action": None,
        })

    taskexplorer_available = _resolve_external_tool("taskexplorer") is not None
    if not taskexplorer_available:
        findings.append({
            "severity": "INFO",
            "message": "TaskExplorer corroboration backend unavailable",
            "action": None,
        })
    else:
        findings.append({
            "severity": "INFO",
            "message": (
                "TaskExplorer is installed; broad injection audit skips "
                "per-process TaskExplorer runs to keep the full-host scan "
                "bounded. Use Deep Process Triage for selected-PID "
                "TaskExplorer corroboration."
            ),
            "action": None,
        })

    suspicious = []
    requested_pids = os.environ.get("SECPROCMON_INJECTION_AUDIT_PIDS", "").strip()
    pid_source = []
    if requested_pids:
        for token in requested_pids.split(","):
            token = token.strip()
            if not token:
                continue
            try:
                pid_source.append(int(token))
            except ValueError:
                continue
    if not pid_source:
        pid_source = _list_all_pids()

    for pid in pid_source:
        if pid <= 0:
            continue
        finding = _audit_injection_antidebug_pid(
            pid,
    ...
    suspicious.sort(
        key=lambda f: {"CRITICAL": 0, "HIGH": 1, "MEDIUM": 2, "INFO": 3, "OK": 4}.get(
            f.get("severity", "INFO"), 99)
    )
    findings.extend(suspicious[:60])

    if not suspicious:
        findings.append({
            "severity": "OK",
            "message": "No injection / anti-debug signals detected in visible processes",
            "action": None,
        })
    return findings
...
def _audit_binary_authorization():
    """Optional binary authorization telemetry sourced from Santa."""
    findings = []
    status = _santa_status_snapshot(timeout=10)
    cfg = _santa_config_snapshot()
    telemetry = _read_santa_telemetry(limit=8)
    if status.get("error") and not status.get("raw"):
        findings.append({
            "severity": "INFO",
            "message": f"Santa telemetry backend unavailable: {status['error']}",
            "action": None,
        })
        return findings
    ...
    if status.get("running"):
        sev = "OK" if mode in ("MONITOR", "LOCKDOWN") else "INFO"
        findings.append({
            "severity": sev,
            "message": f"Santa telemetry backend healthy (mode={mode})",
            "evidence": (status.get("raw") or "")[:800],
            "action": None,
        })
    else:
        findings.append({
            "severity": "MEDIUM",
            "message": "Santa is installed but its telemetry daemon is not healthy",
            "evidence": (status.get("error") or status.get("raw") or "")[:800],
            "action": None,
        })

    telemetry_cfg = cfg.get("telemetry") or []
    log_bits = [
        f"domain={cfg.get('domain') or '(default)'}",
        f"log_type={cfg.get('event_log_type') or 'file'}",
        f"log_path={cfg.get('event_log_path') or _SANTA_DEFAULT_EVENT_LOG_PATH}",
        (
            "telemetry="
            + (", ".join(telemetry_cfg) if telemetry_cfg else "(default/unspecified)")
        ),
    ]
    if cfg.get("file_changes_regex"):
        log_bits.append(f"file_changes_regex={cfg['file_changes_regex']}")
    if cfg.get("file_changes_prefix_filters"):
        log_bits.append(
            "file_changes_prefix_filters="
            + ", ".join(cfg["file_changes_prefix_filters"][:4])
        )
    findings.append({
        "severity": "INFO",
        "message": "Santa telemetry configuration snapshot",
        "evidence": "\n".join(log_bits),
        "action": None,
    })
    ...
    if telemetry.get("error"):
        findings.append({
            "severity": "INFO",
            "message": "Santa recent telemetry unavailable from disk",
            "evidence": telemetry["error"][:800],
            "action": None,
        })
    else:
        counts = telemetry.get("counts") or {}
        decisions = telemetry.get("decisions") or {}
        summary = [
            f"events_read={telemetry.get('events_read', 0)}",
            f"Execution={counts.get('Execution', 0)}",
            f"Authentication={counts.get('Authentication', 0)}",
            f"GatekeeperOverride={counts.get('GatekeeperOverride', 0)}",
            f"CodesigningInvalidated={counts.get('CodesigningInvalidated', 0)}",
            f"FileAccess={counts.get('FileAccess', 0)}",
        ]
        if decisions:
            summary.append(
                "decisions="
        ...
        sev = "OK"
        if counts.get("CodesigningInvalidated", 0) > 0:
            sev = "HIGH"
        elif counts.get("GatekeeperOverride", 0) > 0:
            sev = "MEDIUM"
        elif telemetry.get("events_read", 0) == 0:
            sev = "INFO"
        evidence = summary[:]
        evidence.extend((telemetry.get("samples") or [])[:6])
        findings.append({
            "severity": sev,
            "message": "Santa recent telemetry snapshot",
            "evidence": "\n".join(evidence),
            "action": None,
        })

    doctor = _santa_doctor_snapshot(timeout=20)
    if doctor.get("error") and not doctor.get("issues"):
        findings.append({
            "severity": "INFO",
            "message": f"Santa deployment diagnostics unavailable: {doctor['error']}",
            "action": None,
        })
    elif doctor.get("issues"):
        findings.append({
            "severity": "MEDIUM",
            "message": (
                f"Santa deployment diagnostics reported "
                f"{len(doctor['issues'])} issue(s)"
            ),
            "evidence": "\n".join(doctor["issues"][:8]),
            "action": None,
        })
    else:
        findings.append({
            "severity": "OK",
            "message": "Santa deployment diagnostics reported no obvious issues",
            "action": None,
        })
    return findings


def _audit_tool_correlation():
    """Cross-correlate optional backend telemetry with first-party runtime findings."""
    findings = []

    kk = _run_knockknock_scan(timeout=120)
    bb = _read_blockblock_summary(limit=10)
    santa_status = _santa_status_snapshot(timeout=10)
    santa_doctor = _santa_doctor_snapshot(timeout=20)
    santa_telemetry = _read_santa_telemetry(limit=10)
    osquery_rows, osquery_err = _osquery_process_snapshot(timeout=20)
    injection_findings = _audit_injection_antidebug()
    ...
    task_corroborated = [
        f for f in severe_runtime
        if "taskexplorer:" in (f.get("evidence") or "").lower()
    ]
    missing_on_disk = 0
    if osquery_rows:
        for row in osquery_rows.values():
            if str(row.get("on_disk", "1")).lower() in ("0", "false"):
                missing_on_disk += 1

    overview = []
    if kk.get("error"):
        overview.append(f"KnockKnock={kk['error']}")
    else:
        overview.append(
            f"KnockKnock flagged={kk.get('flagged_items', 0)} "
            f"persistent={kk.get('persistent_items', 0)}"
        )
    if bb.get("error"):
        overview.append(f"BlockBlock={bb['error']}")
    else:
        overview.append(
            f"BlockBlock alerts={bb.get('alerts', 0)} "
            f"blocked={bb.get('blocked', 0)}"
        )
    if santa_status.get("error") and not santa_status.get("raw"):
        overview.append(f"Santa={santa_status['error']}")
    else:
        state = "running" if santa_status.get("running") else "not-healthy"
        overview.append(
            f"Santa {state} mode={santa_status.get('mode') or 'UNKNOWN'}"
        )
    if santa_telemetry.get("error"):
        overview.append(f"SantaLog={santa_telemetry['error']}")
    else:
        counts = santa_telemetry.get("counts") or {}
        overview.append(
            "SantaLog "
            f"{santa_telemetry.get('log_type') or 'file'} "
            f"events={santa_telemetry.get('events_read', 0)} "
            f"exec={counts.get('Execution', 0)} "
            f"auth={counts.get('Authentication', 0)} "
            f"gk_override={counts.get('GatekeeperOverride', 0)}"
        )
    if osquery_err:
        overview.append(f"osquery={osquery_err}")
    else:
        overview.append(
            f"osquery processes={len(osquery_rows)} on_disk0={missing_on_disk}"
        )
    overview.append(f"runtime_high={len(severe_runtime)}")
    findings.append({
        "severity": "INFO",
        "message": "Optional backend telemetry overview",
        "evidence": "\n".join(overview),
        "action": None,
    })

    if kk.get("flagged_items", 0) > 0 and bb.get("alerts", 0) > 0:
        evidence = []
        if kk.get("categories"):
            evidence.extend(kk["categories"][:6])
        if bb.get("samples"):
            evidence.extend(bb["samples"][:6])
        findings.append({
            "severity": "HIGH",
            "message": (
                "Optional persistence backends corroborate suspicious "
                "launch-item telemetry"
            ),
            "evidence": "\n".join(evidence),
            "action": None,
        })

    if severe_runtime and (task_corroborated or missing_on_disk > 0):
        evidence = []
        if missing_on_disk > 0:
            evidence.append(
                f"osquery reported {missing_on_disk} process row(s) with on_disk=0"
            )
        if task_corroborated:
            evidence.extend(
                f.get("message", "") for f in task_corroborated[:4]
            )
        findings.append({
            "severity": (
                "CRITICAL"
                if missing_on_disk > 0 and task_corroborated
                else "HIGH"
            ),
            "message": (
                "Runtime stealth signals are corroborated across "
                "the custom injection audit and optional backends "
                "(osquery, TaskExplorer)"
            ),
            "evidence": "\n".join(evidence),
            "action": None,
        })

    santa_counts = santa_telemetry.get("counts") or {}
    if not santa_telemetry.get("error") and (
        santa_counts.get("CodesigningInvalidated", 0) > 0
        or santa_counts.get("GatekeeperOverride", 0) > 0
        or any(
            key in santa_counts for key in (
                "Authentication", "OpenSSH", "ScreenSharing",
                "LoginWindowSession", "LoginLogout",
            )
        )
    ):
        findings.append({
            "severity": (
                "HIGH"
                if santa_counts.get("CodesigningInvalidated", 0) > 0
                else "MEDIUM"
            ),
            "message": (
                "Santa telemetry log corroborates recent execution-control "
                "or operator-access signals"
            ),
            "evidence": "\n".join((santa_telemetry.get("samples") or [])[:8]),
            "action": None,
        })

    if severe_runtime and not santa_status.get("running"):
        findings.append({
            "severity": "MEDIUM",
            "message": (
                "Runtime anomalies are present, but Santa telemetry "
                "did not provide corroboration"
            ),
            "evidence": (
                (santa_status.get("error") or santa_status.get("raw") or "")[:800]
            ),
            "action": None,
        })

    if santa_doctor.get("issues"):
        findings.append({
            "severity": "INFO",
            "message": "Santa deployment issues weaken optional execution telemetry",
            "evidence": "\n".join(santa_doctor.get("issues", [])[:8]),
            "action": None,
        })
    ...
    if not any(f.get("severity") in ("HIGH", "CRITICAL") for f in findings):
        findings.append({
            "severity": "OK",
            "message": "No multi-tool correlations elevated the current host signal",
            "action": None,
        })
    return findings
...
# be extended by dropping JSON files into ~/.mac-tui-procmon-rules.d/.
...
def _load_custom_rules():
    """Load custom rule JSON from ~/.mac-tui-procmon-rules.d."""
    import json as _j
    root = os.path.join(_EFFECTIVE_HOME, ".mac-tui-procmon-rules.d")
    if not os.path.isdir(root):
        return []
    out = []
    for name in os.listdir(root):
...
_LAYER_FOR_AUDIT = {
    ...
    "cve_intelligence": "Library",
    "process_entitlements": "Library",
    "injection_antidebug": "Library",
    "filesystem_integrity": "System",
    "sensitive_paths_delta": "System",
    "keychain": "System",
    "auth_stack": "System",
    "binary_authorization": "System",
    "tool_correlation": "System",
    "packages": "Library",
...
def _all_audit_functions():
    """Ordered list of every host-level audit included in the full audit."""
    return [
...
def _audit_global_score():
    ...
    """
    results = {}
    for key, fn in _all_audit_functions():
        try:
            results[key] = fn()
...
_FULL_AUDIT_REVIEW_PROMPT = (
    "You are a senior macOS DFIR analyst reviewing a secprocmon full-host "
    "security scan.\n"
    ...
    "TOOL-CORRELATED SIGNALS:\n"
    "- specifically mention KnockKnock, BlockBlock, Santa, TaskExplorer, "
    "osquery, the Optional Backend Correlation audit, and the custom "
    "injection/anti-debug audit when present\n"
    "- treat those tools as corroboration backends, not as standalone proof\n"
    "INVESTIGATION PLAN:\n"
    "- exact next steps an operator should take\n"
    "FALSE-POSITIVE NOTES:\n"
    "- benign explanations worth considering\n"
    "Be specific, grounded in the evidence, and do not invent telemetry."
)

_FULL_AUDIT_SYNTHESIS_PROMPT = (
    "You are reconciling multiple independent LLM reviews of the same "
    "secprocmon full-host security scan.\n"
    "Use the raw scan corpus as ground truth. Produce one additive report "
    "that keeps claims supported by the scan, preserves disagreements when "
    "they matter, and notes missing reviewers without guessing.\n"
    "Return exactly this shape:\n"
    "RISK: [LOW|MEDIUM|HIGH|CRITICAL]\n"
    "EXECUTIVE SUMMARY:\n"
    "- 3 to 6 bullets\n"
    ...
    "- ordered findings with severity and why they matter\n"
    "TOOL-CORRELATED SIGNALS:\n"
    "- corroborated signals across native findings and optional backends\n"
    "INVESTIGATION PLAN:\n"
    "- exact next steps\n"
    "FALSE-POSITIVE NOTES:\n"
    "- plausible benign explanations worth checking\n"
    "REVIEW COVERAGE:\n"
    "- which reviewers contributed and which were unavailable\n"
    "Do not invent telemetry. If reviewers disagree, say so plainly."
)


def _run_llm_cli(tool, prompt, input_text, timeout=180):
    """Run a single CLI LLM from the shell, returning text or an error tag."""
    cfg = _HEADLESS_LLM_TOOLS.get(tool)
    if cfg is None:
    ...
    try:
        ...
        try:
            ...
            proc.kill()
            proc.wait()
            return f"[{tool} report timed out after {timeout}s]"
        if proc.returncode != 0:
            detail = (stderr.decode("utf-8", errors="replace").strip()
                      or stdout.decode("utf-8", errors="replace").strip()
                      or "(no output)")
            return f"[{tool} CLI error (rc={proc.returncode}): {detail[:240]}]"
        return stdout.decode("utf-8", errors="replace").strip()
    except FileNotFoundError:
        return f"[{tool} CLI not found]"
    except OSError as e:
        return f"[{tool} CLI error: {e}]"


def _run_llm_reviews_parallel(prompt, input_text, tools=("claude", "codex", "gemini"),
                              progress=None):
    """Run every configured headless reviewer and return tool -> response."""
    results = {}
    threads = []

    def _worker(tool):
        if progress:
            progress(f"review {tool}: starting")
        result = _run_llm_cli(tool, prompt, input_text, timeout=180)
        results[tool] = result
        if progress:
            if result.lstrip().startswith("["):
                progress(f"review {tool}: unavailable/error {result[:120]}")
            else:
                progress(f"review {tool}: complete")
    ...
    for tool in tools:
        if tool not in results:
            results[tool] = f"[{tool} review timed out after 180s]"
            if progress:
                progress(f"review {tool}: timed out after 180s")
    return results


def _parse_review_risk(text):
    """Extract LOW/MEDIUM/HIGH/CRITICAL from review text."""
    for line in (text or "").splitlines():
        raw = line.strip()
        upper = raw.upper()
        if upper.startswith("RISK:") or upper.startswith("CONSENSUS_RISK:"):
...
def _split_review_sections(text):
    """Parse simple all-caps colon headers into bullet sections."""
    headers = (
        "EXECUTIVE SUMMARY",
        "PRIORITY FINDINGS",
        "TOOL-CORRELATED SIGNALS",
...
def _append_unique_items(dest, items, prefix=""):
    """Append items to dest, de-duplicating by normalized content."""
    seen = {
    ...
    for item in items:
        if not item:
            continue
        rendered = f"{prefix}{item}" if prefix else item
        key = re.sub(r"\s+", " ", rendered.lower()).strip()
        if key in seen:
            continue
...
def _local_additive_full_audit_review(score, results, reviews):
    """Build a local additive review from the raw scan + available reviewer text."""
    available = {
        tool: text for tool, text in reviews.items()
    ...
    parsed_risks = [
        _parse_review_risk(text)
        for text in available.values()
        if _parse_review_risk(text)
    ]
    additive_risk = (
        max(parsed_risks, key=lambda item: rank.get(item, -1))
        if parsed_risks
        else {"GREEN": "LOW", "YELLOW": "MEDIUM",
              "ORANGE": "HIGH", "RED": "CRITICAL"}[_severity_band(score["global"])]
    )
    ...
    for tool in ("claude", "codex", "gemini"):
        text = available.get(tool)
        if not text:
            continue
        parsed = _split_review_sections(text)
        for name in (
            "EXECUTIVE SUMMARY",
            "PRIORITY FINDINGS",
            "TOOL-CORRELATED SIGNALS",
    ...
    if not sections["EXECUTIVE SUMMARY"]:
        sections["EXECUTIVE SUMMARY"].append(
            f"Global security score is {score['global']}/100 "
            f"[{_severity_band(score['global'])}]."
        )
        if score["fix_first"]:
            audit_key, finding = score["fix_first"][0]
            sections["EXECUTIVE SUMMARY"].append(
                f"Highest-priority native finding: "
                f"[{audit_key}] [{finding.get('severity')}] {finding.get('message')}"
            )

    if not sections["PRIORITY FINDINGS"]:
        for audit_key, finding in score["fix_first"][:8]:
            sections["PRIORITY FINDINGS"].append(
                f"[{audit_key}] [{finding.get('severity')}] {finding.get('message')}"
            )
        if not sections["PRIORITY FINDINGS"]:
            sections["PRIORITY FINDINGS"].append(
                "No HIGH/CRITICAL actionable findings were surfaced by the native scan."
            )

    if not sections["TOOL-CORRELATED SIGNALS"]:
        for key in ("tool_correlation", "binary_authorization", "injection_antidebug"):
            for finding in results.get(key, [])[:4]:
                sections["TOOL-CORRELATED SIGNALS"].append(
                    f"[{key}] [{finding.get('severity')}] {finding.get('message')}"
                )
        if not sections["TOOL-CORRELATED SIGNALS"]:
            sections["TOOL-CORRELATED SIGNALS"].append(
                "No optional-backend corroboration was available."
            )

    if not sections["INVESTIGATION PLAN"]:
        if score["fix_first"]:
            sections["INVESTIGATION PLAN"].append(
                "Validate and remediate the top Fix-First findings in severity order."
            )
        sections["INVESTIGATION PLAN"].append(
            "Re-run the automated scan after remediation to confirm the host posture changed."
        )

    if not sections["FALSE-POSITIVE NOTES"]:
        sections["FALSE-POSITIVE NOTES"].append(
            "Recently changed admin tooling, MDM, developer shells, and sanctioned browser extensions can elevate findings."
        )
        sections["FALSE-POSITIVE NOTES"].append(
            "Treat optional backends as corroboration only; absence of Santa/osquery/TaskExplorer does not clear a finding."
        )

    sections["REVIEW COVERAGE"].append(
    ...
    if missing:
        sections["REVIEW COVERAGE"].append(
            ...
            + ", ".join(f"{tool}={reviews[tool]}" for tool in sorted(missing))
        )

    lines = [f"RISK: {additive_risk}"]
    for name in (
        "EXECUTIVE SUMMARY",
        "PRIORITY FINDINGS",
        "TOOL-CORRELATED SIGNALS",
        ...
        lines.append(f"{name}:")
        for item in sections[name][:12]:
            lines.append(f"- {item}")
    return "\n".join(lines)


def _synthesize_full_audit_reviews(score, results, llm_body, reviews, progress=None):
    """Combine all available reviewer outputs into one additive report."""
    available = {
        tool: text for tool, text in reviews.items()
        if text and not text.lstrip().startswith("[")
    }
    if not available:
        if progress:
            progress("synthesis: no external reviews available, using local merge")
        return None, _local_additive_full_audit_review(score, results, reviews)
    ...
    for tool in ("claude", "codex", "gemini"):
        text = reviews.get(tool, "")
        combined.append(f"=== {tool.upper()} ===")
        combined.append(text or f"[{tool} produced no output]")
        combined.append("")
    input_text = "\n".join(combined)
    ...
    for tool in ("claude", "codex", "gemini"):
        if tool not in available:
            continue
        if progress:
            progress(f"synthesis: trying {tool}")
        synth = _run_llm_cli(tool, _FULL_AUDIT_SYNTHESIS_PROMPT, input_text, timeout=180)
        if not synth.startswith("["):
            if progress:
                progress(f"synthesis: complete with {tool}")
            return tool, synth
        if progress:
            progress(f"synthesis: {tool} unavailable/error {synth[:120]}")
    if progress:
        progress("synthesis: external synthesis unavailable, using local merge")
    return None, _local_additive_full_audit_review(score, results, reviews)


def _full_audit_results(progress=None, heartbeat_interval=10):
    """Run the full audit suite and return {audit_key: findings}."""
    results = {}
    audits = list(_all_audit_functions())
    total = len(audits)
    for idx, (key, fn) in enumerate(audits, 1):
        if progress:
            progress(f"audit {idx}/{total}: {key} starting")
        stop_heartbeat = None
        if progress:
            stop_heartbeat = threading.Event()
            started_at = time.monotonic()

            def _heartbeat(stop=stop_heartbeat, audit_idx=idx,
                           audit_key=key, started=started_at):
                while not stop.wait(heartbeat_interval):
                    elapsed = int(time.monotonic() - started)
                    progress(
                        f"audit {audit_idx}/{total}: {audit_key} still running "
                        f"({elapsed}s)")
        ...
        try:
            ...
            if progress:
                progress(f"audit {idx}/{total}: {key} error {e}")
        else:
            if progress:
                progress(
                    f"audit {idx}/{total}: {key} complete "
                    f"({len(results[key])} findings)")
        finally:
...
def _build_full_audit_llm_body(results, score):
    """Build the full-audit corpus sent to Claude/Codex/Gemini."""
    lines = [
        "SECPROCMON FULL HOST AUDIT",
        f"GLOBAL SCORE: {score['global']}",
        f"LAYER SCORES: {score['layers']}",
        "",
    ]
...
def _full_audit_totals(results):
    """Aggregate severity counts across the full audit suite."""
...
def _render_automated_scan_summary(score, results, additive_report, reviews):
    """Render the short operator summary that sits at the top of the report."""
    totals = _full_audit_totals(results)
    risk = _parse_review_risk(additive_report)
    if not risk:
        risk = {"GREEN": "LOW", "YELLOW": "MEDIUM",
                "ORANGE": "HIGH", "RED": "CRITICAL"}[_severity_band(score["global"])]
    available = [
        tool for tool in ("claude", "codex", "gemini")
        if reviews.get(tool) and not reviews[tool].lstrip().startswith("[")
    ]
    missing = [
        tool for tool in ("claude", "codex", "gemini")
        if tool not in available
    ...
    lines = [
        "SUMMARY:",
        f"- Global security score: {score['global']}/100 [{_severity_band(score['global'])}]",
        f"- Overall reviewed risk: {risk}",
        (
            f"- Findings: {totals['total_findings']} total "
            f"(CRITICAL {totals['counts']['CRITICAL']}, "
            f"HIGH {totals['counts']['HIGH']}, "
            f"MEDIUM {totals['counts']['MEDIUM']}, "
            f"INFO {totals['counts']['INFO']}, "
            f"OK {totals['counts']['OK']})"
        ),
        f"- Actionable findings: {totals['actionable']}",
        f"- Audits completed: {totals['audits_run']}",
        "- Reviewers used: " + (", ".join(available) if available else "(none)"),
    ]
    if missing:
        lines.append("- Missing reviewers: " + ", ".join(missing))
    if score["fix_first"]:
        for audit_key, finding in score["fix_first"][:3]:
            lines.append(
                f"- Fix first: [{audit_key}] [{finding.get('severity')}] {finding.get('message')}"
            )
    else:
        lines.append("- Fix first: no HIGH/CRITICAL actionable findings")
    return "\n".join(lines)
...
def _clean_report_text(text):
    """Remove terminal controls from reviewer text before rendering."""
    cleaned = _ANSI_CONTROL_RE.sub("", str(text or ""))
    return cleaned.replace("\r", "")


def _wrap_security_report(lines, width=100):
    """Wrap long prose while preserving report structure and bullet indents."""
    wrapped = []
    for raw in lines:
        line = _clean_report_text(raw).rstrip()
        if not line:
            wrapped.append("")
            continue
        stripped = line.lstrip()
        leading = line[:len(line) - len(stripped)]
        if len(line) <= width or set(line) <= {"=", "-", "─"}:
            wrapped.append(line)
            continue
        if stripped.startswith("- "):
            initial = leading + "- "
            ...
            initial = leading + prefix + " "
            subsequent = leading + " " * (len(prefix) + 1)
        else:
            initial = leading
            subsequent = leading + "  "
            body = stripped
...
def _severity_sort_key(finding):
    ...
    return (
        rank.get(finding.get("severity", "INFO"), 99),
        0 if finding.get("action") else 1,
        finding.get("message", ""),
    )


def _top_security_findings(results, limit=12, severities=("CRITICAL", "HIGH")):
    """Return worst native findings across all audits, including non-remediable rows."""
    rows = []
    for audit_key, findings in results.items():
        for finding in findings:
            if finding.get("severity") in severities:
                rows.append((audit_key, finding))
    rows.sort(key=lambda item: _severity_sort_key(item[1]))
    return rows[:limit]


def _all_security_findings(results):
    rows = []
    ordinal = 0
    for audit_key, findings in results.items():
        for finding in findings:
            if finding.get("severity") == "OK":
                continue
            ordinal += 1
            rows.append((audit_key, finding, ordinal))
    return rows


def _counts_for_findings(findings):
    counts = {"CRITICAL": 0, "HIGH": 0, "MEDIUM": 0, "INFO": 0, "OK": 0}
    for finding in findings:
        sev = finding.get("severity", "INFO")
        if sev in counts:
...
def _format_layer_scores(score):
    parts = []
    for layer in ("Network", "System", "Kernel", "Library"):
        value = score["layers"].get(layer, 0)
        sev = {
            "GREEN": "OK",
            "YELLOW": "MEDIUM",
            "ORANGE": "HIGH",
            "RED": "CRITICAL",
        }[_severity_band(value)]
        parts.append(f"{layer} {value}/100 [{sev}]")
    return " | ".join(parts)


def _reviewer_status_line(reviews):
    statuses = []
    for tool in ("claude", "codex", "gemini"):
        text = _clean_report_text(reviews.get(tool, ""))
        if not text or text.lstrip().startswith("["):
            statuses.append(f"{tool}: unavailable")
            continue
        statuses.append(f"{tool}: {_parse_review_risk(text) or 'reviewed'}")
    return " | ".join(statuses)


def _section_items(text, section, limit):
    sections = _split_review_sections(_clean_report_text(text))
    return sections.get(section, [])[:limit]


def _append_numbered_section(lines, title, items, empty_message, limit=None):
    lines.extend(["", title])
    ...
    for idx, item in enumerate(selected, 1):
        rendered = item.strip()
        if re.match(r"^\d+\.\s+", rendered):
            lines.append(rendered)
        else:
            lines.append(f"{idx}. {rendered}")
    if limit and len(items) > limit:
        lines.append(f"- (+{len(items) - limit} more omitted from primary view)")


def _reviewer_excerpt_lines(tool, text, max_items=5):
    clean = _clean_report_text(text).strip()
    if not clean:
        return [f"== {tool.upper()} ==", f"[{tool} produced empty output]"]
    if clean.lstrip().startswith("["):
        return [f"== {tool.upper()} ==", clean]
    risk = _parse_review_risk(clean) or "reviewed"
    items = []
    for section in ("EXECUTIVE SUMMARY", "PRIORITY FINDINGS"):
        items.extend(_section_items(clean, section, max_items))
        if len(items) >= max_items:
            break
    if not items:
        items = [line.strip() for line in clean.splitlines()
                 if line.strip() and not line.upper().startswith("RISK:")][:max_items]
    lines = [f"== {tool.upper()} [{risk}] =="]
    for item in items[:max_items]:
        lines.append(f"- {item}")
    if len(clean.splitlines()) > max_items + 2:
        lines.append("- (raw reviewer output trimmed from primary report)")
    return lines


def _compose_full_audit_report(score, results, reviews, synth_tool, additive_report):
    """Render a readable, grouped automated scan report from completed parts."""
    additive_report = _clean_report_text(additive_report)
    totals = _full_audit_totals(results)
    risk = _parse_review_risk(additive_report)
    if not risk:
        risk = {"GREEN": "LOW", "YELLOW": "MEDIUM",
    ...
    top_native = _top_security_findings(results, limit=12)
    executive = _section_items(additive_report, "EXECUTIVE SUMMARY", 6)
    priority = _section_items(additive_report, "PRIORITY FINDINGS", 10)
    tool_signals = _section_items(additive_report, "TOOL-CORRELATED SIGNALS", 8)
    investigation = _section_items(additive_report, "INVESTIGATION PLAN", 10)
    false_positive = _section_items(additive_report, "FALSE-POSITIVE NOTES", 8)
    review_coverage = _section_items(additive_report, "REVIEW COVERAGE", 8)
    available_reviewers = [
        tool for tool in ("claude", "codex", "gemini")
        if reviews.get(tool) and not _clean_report_text(reviews[tool]).lstrip().startswith("[")
    ]
    missing_reviewers = [
        tool for tool in ("claude", "codex", "gemini")
        if tool not in available_reviewers
    ]
    ...
    lines = [
        ...
        "READ THIS FIRST",
        (
            f"[{risk_tag}] Reviewed risk: {risk} | "
            f"Global score: {score['global']}/100 [{band}] | "
            f"Findings: {totals['total_findings']} | "
            f"Actionable: {totals['actionable']}"
        ),
        f"Overall reviewed risk: {risk}",
        "Layer scores: " + _format_layer_scores(score),
        "Reviewers used: " + (
            ", ".join(available_reviewers) if available_reviewers else "(none)"
        ),
        "Missing reviewers: " + (
            ", ".join(missing_reviewers) if missing_reviewers else "(none)"
        ),
        "Reviewer risk votes: " + _reviewer_status_line(reviews),
        (
            "Additive synthesis: "
            + (synth_tool if synth_tool else "local merge of available reviewer outputs")
        ),
        "",
        "SEVERITY TOTALS",
        _format_severity_counts(totals["counts"]),
        "",
        "FIX FIRST - NATIVE EVIDENCE",
    ]
    if top_native:
        for idx, (audit_key, finding) in enumerate(top_native, 1):
            action = " [actionable]" if finding.get("action") else ""
            if idx <= 3:
                lines.append(
                    f"Fix first: [{audit_key}] "
                    f"[{finding.get('severity', 'INFO')}] "
                    f"{finding.get('message', '')}"
                )
            lines.append(
                f"{idx}. [{finding.get('severity', 'INFO')}] "
                f"{audit_key}: {finding.get('message', '')}{action}"
            )
    else:
        lines.append("- No CRITICAL/HIGH native findings.")

    _append_numbered_section(
        lines,
        "EXECUTIVE SUMMARY",
        executive,
        "No reviewer executive summary was available.",
        limit=6,
    )
    _append_numbered_section(
        lines,
        ...
        priority,
        "No reviewer priority findings were available.",
        limit=10,
    )
    _append_numbered_section(
        lines,
        "TOOL-CORRELATED SIGNALS",
        tool_signals,
        "No tool-correlation narrative was available.",
        limit=8,
    )
    _append_numbered_section(
        lines,
        "INVESTIGATION PLAN",
        investigation,
        "Validate and remediate the Fix First findings, then re-run the scan.",
        limit=10,
    )
    _append_numbered_section(
        lines,
        "FALSE-POSITIVE / VERIFY BEFORE ACTION",
        false_positive,
        "No false-positive guidance was available.",
        limit=8,
    )
    _append_numbered_section(
        lines,
        "REVIEW COVERAGE",
        review_coverage,
        "Reviewer coverage details were not provided by the synthesis.",
        limit=8,
    )

    lines.extend(["", "PER-AUDIT HEATMAP"])
    for key, findings in results.items():
        counts = _counts_for_findings(findings)
        layer = _LAYER_FOR_AUDIT.get(key, "Library")
        lines.append(f"- {key} ({layer}): {_format_severity_counts(counts)}")

    lines.extend([
        "",
        "REVIEWER EXCERPTS (TRIMMED)",
        (
            "Full raw reviewer dumps are intentionally excluded from the "
            "primary TUI report to keep the output readable."
        ),
    ])
    for tool in ("claude", "codex", "gemini"):
        lines.append("")
        lines.extend(_reviewer_excerpt_lines(
            tool, reviews.get(tool, f"[{tool} review not run]")))

    return "\n".join(_wrap_security_report(lines))


_REPORT_LOCAL_PATH_RE = re.compile(
    r"(/(?:Applications|Library|System|Users|private|opt|usr|bin|sbin|etc|var|tmp|Volumes)"
    r"[^\s<>'\"`|]*)"
)

_AUDIT_REPORT_TITLES = {
    "network": "Network Exposure",
    "dns": "DNS / Proxy / MDM",
    "persistence": "Persistence",
    "system_hardening": "System Hardening",
    "kernel_boot": "Kernel / Boot Integrity",
    "patch_posture": "OS Patch Posture",
    "tcc": "TCC Grants",
    "browser_exts": "Browser Extensions",
    "usb_hid": "USB / HID",
    "shell_dotfiles": "Shell Dotfiles",
    "installed_software": "Installed Software Trust",
    "cve_intelligence": "CVE Intelligence",
    "process_entitlements": "Process Entitlements",
    "injection_antidebug": "Injection / Anti-Debug",
    "filesystem_integrity": "Filesystem Integrity",
    "sensitive_paths_delta": "Sensitive Paths Delta",
    "keychain": "Keychain & Credential Hygiene",
    "auth_stack": "Authentication Stack",
    "binary_authorization": "Binary Authorization",
    "tool_correlation": "Optional Backend Correlation",
...
def _html_value(value):
    ...
    if isinstance(value, str):
        return _clean_report_text(value)
    try:
        return _json.dumps(value, ensure_ascii=True, sort_keys=True)
    except Exception:
        return _clean_report_text(str(value))


def _html_with_local_links(text):
    """Escape text and link common absolute local paths in the HTML report."""
    clean = _clean_report_text(text)
    parts = []
    cursor = 0
    for match in _REPORT_LOCAL_PATH_RE.finditer(clean):
        raw_path = match.group(1)
        path = raw_path.rstrip(".,;:)]}")
        trailing = raw_path[len(path):]
        if not path:
            continue
        parts.append(_html.escape(clean[cursor:match.start(1)]))
        href = "file://" + _urlparse.quote(path)
        parts.append(
            f'<a class="path-link" href="{href}">{_html.escape(path)}</a>'
            f"{_html.escape(trailing)}"
        )
        cursor = match.end(1)
    parts.append(_html.escape(clean[cursor:]))
    return "".join(parts)


def _html_severity_class(severity):
    sev = str(severity or "INFO").upper()
    if sev == "LOW":
        sev = "OK"
    if sev not in ("CRITICAL", "HIGH", "MEDIUM", "INFO", "OK"):
        sev = "INFO"
    return sev.lower()


def _html_badge(label, severity="INFO"):
    return (
        f'<span class="badge sev-{_html_severity_class(severity)}">'
        f"{_html.escape(str(label))}</span>"
    )


def _band_to_severity(band):
    return {
        "GREEN": "OK",
        "YELLOW": "MEDIUM",
        "ORANGE": "HIGH",
...
def _html_counts_badges(counts):
    badges = []
    for sev in ("CRITICAL", "HIGH", "MEDIUM", "INFO", "OK"):
        count = counts.get(sev, 0)
        if count:
            badges.append(_html_badge(f"{sev} {count}", sev))
    return " ".join(badges) if badges else '<span class="muted">none</span>'


def _html_severity_vector(counts):
    total = sum(counts.get(sev, 0) for sev in ("CRITICAL", "HIGH", "MEDIUM", "INFO", "OK"))
    if total <= 0:
        return '<div class="severity-vector empty"><span></span></div>'
    segments = []
    for sev in ("CRITICAL", "HIGH", "MEDIUM", "INFO", "OK"):
        count = counts.get(sev, 0)
        if not count:
            continue
        pct = max(2, round((count / total) * 100, 2))
        segments.append(
            f'<span class="seg sev-bg-{_html_severity_class(sev)}" '
            f'style="width:{pct}%" title="{_html.escape(sev)} {count}"></span>'
        )
    return '<div class="severity-vector">' + "".join(segments) + "</div>"


def _audit_report_title(key):
    return _AUDIT_REPORT_TITLES.get(key, key.replace("_", " ").title())


def _html_list(items, empty_message, ordered=False):
    if not items:
        return f'<p class="muted">{_html.escape(empty_message)}</p>'
    tag = "ol" if ordered else "ul"
    body = "\n".join(
        f"<li>{_html_with_local_links(item)}</li>"
        for item in items
    )
    return f"<{tag}>\n{body}\n</{tag}>"


def _html_finding_detail(finding):
    action = _html_value(finding.get("action")).strip()
    evidence = _html_value(finding.get("evidence")).strip()
    detail_parts = []
    if action:
        detail_parts.append(
            '<div class="action-pill">Actionable</div>'
            f'<div class="detail-block"><strong>Action:</strong> '
            f"{_html_with_local_links(action)}</div>"
        )
    if evidence:
        detail_parts.append(
            "<details><summary>Evidence</summary>"
            f"<pre>{_html.escape(evidence)}</pre></details>"
        )
    return "\n".join(detail_parts) or '<span class="muted">No extra evidence.</span>'


def _html_copy_button(text, label):
    return (
        '<button type="button" class="copy-button" '
        f'data-copy="{_html.escape(_clean_report_text(text), quote=True)}">'
...
def _html_attr(text):
    return _html.escape(_clean_report_text(text), quote=True)


def _finding_text_blob(finding):
    return "\n".join(
        _html_value(finding.get(key)).strip()
        for key in ("message", "action", "evidence")
        if _html_value(finding.get(key)).strip()
    )


def _finding_paths(finding):
    paths = []
    for match in re.finditer(r"(?<![\w.-])(/[^\s\"'<>`|;&]+)", _finding_text_blob(finding)):
...
def _finding_pids(finding):
    ...
    for pattern in (r"\bPID\s*[:=]?\s*(\d+)\b", r"\bpid\s*=\s*(\d+)\b"):
...
def _finding_needs_root(audit_key, finding):
    text = _finding_text_blob(finding).lower()
    root_audits = {
        "auth_stack", "binary_authorization", "filesystem_integrity",
        "kernel_boot", "network", "persistence", "sensitive_paths_delta",
        "system_hardening", "tool_correlation",
    }
    privileged_paths = (
        "/library/", "/system/", "/etc/", "/private/etc/", "/var/db/",
        "/var/root/", "/usr/libexec/", "/library/privilegedhelpertools/",
        "/library/launchdaemons/", "/library/security/",
    )
    if audit_key in root_audits:
        return True
    if any(path in text for path in privileged_paths):
        return True
    if audit_key == "injection_antidebug" and (
        _finding_pids(finding) or "vmmap" in text or "lsof" in text
    ):
        return True
    if "firewall" in text or "launchdaemon" in text or "privilegedhelper" in text:
        return True
    return False


def _shell_cmd(command, needs_root=False):
    is_root = False
    try:
        is_root = os.geteuid() == 0
    except AttributeError:
        is_root = False
    if needs_root and not is_root and not command.lstrip().startswith("sudo "):
        return "sudo " + command
    return command


def _audit_shell_command(audit_key, needs_root=False):
    return _shell_cmd(f"python3 -m secauditor audit {shlex.quote(audit_key)}", needs_root)


def _full_scan_shell_command(needs_root=False):
    return _shell_cmd("python3 -m secauditor full-scan", needs_root)


def _remediation_command_plan(audit_key, finding):
    severity = finding.get("severity", "INFO")
    message = _html_value(finding.get("message")).strip()
    action = _html_value(finding.get("action")).strip()
    paths = _finding_paths(finding)
    pids = _finding_pids(finding)
    needs_root = _finding_needs_root(audit_key, finding)
    try:
        running_as_root = os.geteuid() == 0
    except AttributeError:
        running_as_root = False
    if needs_root and running_as_root:
        root_note = "yes - SecAuditor is already running as root"
    elif needs_root:
        root_note = "yes - privileged commands are prefixed with sudo"
    else:
        root_note = "not inferred - add sudo manually if your local permissions require it"
    lines = [
        "# secprocmon remediation commands",
        "# Review before running; preserve evidence before applying changes.",
        f"# Audit: {audit_key}",
        f"# Severity: {severity}",
        f"# Finding: {message}",
        f"# Root required: {root_note}",
        "",
        "# 1. Reproduce and capture current evidence",
        _audit_shell_command(audit_key, needs_root),
    ]

    for pid in pids[:4]:
        lines.extend([
            _shell_cmd(f"ps -p {shlex.quote(pid)} -o pid,ppid,user,etime,command", False),
            _shell_cmd(f"lsof -p {shlex.quote(pid)} | head -200", needs_root),
            _shell_cmd(f"vmmap {shlex.quote(pid)} | head -160", needs_root),
        ])

    for path in paths[:5]:
        qpath = shlex.quote(path)
        lines.extend([
            _shell_cmd(f"stat -f '%Sp %Su:%Sg %N' {qpath}", needs_root),
            _shell_cmd(f"ls -ldeO@ {qpath}", needs_root),
            _shell_cmd(f"codesign -dv --verbose=4 {qpath} 2>&1 || true", needs_root),
            _shell_cmd(f"shasum -a 256 {qpath} 2>/dev/null || true", needs_root),
        ])

    lines.extend(["", "# 2. Remediate"])
    if action:
        lines.append(f"# Report action: {action}")

    lower_blob = _finding_text_blob(finding).lower()
    if audit_key == "keychain" and paths:
        for path in paths[:3]:
            lines.append(_shell_cmd(f"chmod 600 {shlex.quote(path)}", needs_root))
    elif audit_key == "auth_stack" and paths:
        lines.extend([
            "# Only after confirming the component is untrusted:",
            _shell_cmd('mkdir -p "$HOME/.secprocmon-quarantine"', needs_root),
        ])
        for path in paths[:2]:
            lines.append(_shell_cmd(
                f"mv {shlex.quote(path)} \"$HOME/.secprocmon-quarantine/\"",
                needs_root,
            ))
    elif audit_key == "network":
        if "firewall" in lower_blob:
            lines.append(_shell_cmd(
                "/usr/libexec/ApplicationFirewall/socketfilterfw --setglobalstate on",
                True,
            ))
        if "screen sharing" in lower_blob or "remote management" in lower_blob:
            lines.append("# Disable Screen Sharing / Remote Management in System Settings if not required.")
    elif audit_key == "binary_authorization":
        lines.extend([
            _shell_cmd("santactl status", needs_root),
            _shell_cmd("launchctl kickstart -k system/com.northpolesec.santa.daemon", True),
        ])
    elif audit_key == "injection_antidebug" and pids:
        for pid in pids[:4]:
            lines.append(
                f"# If confirmed malicious after evidence capture: "
                f"{_shell_cmd(f'kill -TERM {shlex.quote(pid)}', needs_root)}"
            )
    elif audit_key == "installed_software" and paths:
        for path in paths[:3]:
            qpath = shlex.quote(path)
            lines.extend([
                f"# Reinstall or remove {qpath} from the trusted vendor source if signature checks fail.",
                _shell_cmd(f"spctl --assess --type execute --verbose=4 {qpath} 2>&1 || true", needs_root),
            ])
    else:
        lines.append("# Apply the report action above, then run verification below.")

    lines.extend([
        "",
        "# 3. Verify after remediation",
        _audit_shell_command(audit_key, needs_root),
        _full_scan_shell_command(needs_root),
    ])
    return "\n".join(lines).strip() + "\n"


def _ai_vulnerability_profile(audit_key, finding):
    """Return concise, report-local AI-style guidance for one finding."""
    text = " ".join(
        _html_value(finding.get(key)).lower()
        for key in ("message", "action", "evidence")
    )
    action = _html_value(finding.get("action")).strip()
    profile = {
        "read": (
            "secprocmon found host state that can affect confidentiality, "
            "integrity, persistence, or exposed attack surface."
        ),
        "likely": (
            "This can come from admin work, vendor software, developer tooling, "
            "or a partial security-tool deployment."
        ),
        "fix": action or "Validate the evidence, remove the unsafe state, then re-run the source audit.",
        "verify": "Re-run the source audit and confirm the same finding no longer appears.",
    }

    if audit_key == "injection_antidebug":
        if "missing" in text or "on_disk=0" in text or "not on disk" in text:
            profile.update({
                "read": (
                    "A live process is reporting an executable path that is no longer backed by a file. "
                    "That is a high-signal runtime anomaly until the real path and parent chain are proven."
                ),
                "likely": (
                    "Most benign cases are app updaters, deleted-but-still-running binaries, or truncated paths "
                    "for apps with spaces in their names. Malicious cases include deleted droppers or process tampering."
                ),
                "verify": (
                    "Capture ps, osquery, lsof, vmmap, parent PID, mapped libraries, and sockets before killing it."
                ),
            })
        else:
            profile.update({
                "read": (
                    "The process has injection, anti-debug, JIT, or weakened runtime indicators. "
                    "Those are expected for some developer runtimes but dangerous when unexpected."
                ),
                "likely": (
                    "Node, browsers, Electron, debuggers, and AI CLIs often create this signal. "
                    "Unknown parents, unexpected DYLD variables, or network activity raise the risk."
                ),
                "verify": "Confirm the parent chain, command line, entitlements, loaded libraries, and owner.",
            })
    elif audit_key == "auth_stack":
        profile.update({
            "read": (
                "The finding sits in the macOS authentication or privilege path. "
                "Unsigned or recently changed auth components can capture credentials or weaken elevation."
            ),
            "likely": (
                "Common benign sources are remote-access products, enterprise agents, or OS updates. "
                "Anything unsigned, ad-hoc, or owner/mode-mismatched should be treated as suspicious."
            ),
            "verify": "Check codesign, Team ID, hash, owner, mode, mtime, package ownership, and a clean-host baseline.",
        })
    elif audit_key == "keychain":
        profile.update({
            "read": (
                "A keychain database is readable more broadly than it should be, allowing another local account "
                "or compromised process to copy encrypted secrets for offline attack."
            ),
            "likely": (
                "This usually comes from migration, backup/sync tools, manual chmod, or imported keychains "
                "that inherited permissive permissions."
            ),
            "verify": "Confirm the mode is 600 after remediation and review access around the permission-change time.",
        })
    elif audit_key == "network":
        profile.update({
            "read": (
                "The host has reachable services or weaker network controls. Remote-control services are especially "
                "sensitive when the firewall is off."
            ),
            "likely": (
                "This is often intentional for Screen Sharing, ARD, VPN, development servers, or collaboration tools. "
                "It is only safe if restricted to trusted users and networks."
            ),
            "verify": "Re-run network exposure and confirm listeners, firewall state, and sharing settings match intent.",
        })
    elif audit_key == "tcc":
        profile.update({
            "read": (
                "Apps with Accessibility, Screen Recording, Input Monitoring, or automation grants can observe or "
                "control sensitive user activity."
            ),
            "likely": (
                "Meeting apps, remote-support tools, launchers, IDEs, and virtualization products commonly request "
                "these grants, but stale grants are unnecessary risk."
            ),
            "verify": "Open Privacy & Security settings and confirm only actively used apps retain the grant.",
        })
    elif audit_key == "browser_exts":
        profile.update({
            "read": (
                "A browser extension has broad host, scripting, debugger, password, or native-messaging access. "
                "Those permissions can expose browsing data if the extension or account is compromised."
            ),
            "likely": (
                "SEO, password-manager, AI, ad-blocking, and productivity extensions often request broad access. "
                "The risk is highest for extensions you no longer use."
            ),
            "verify": "Remove stale extensions and confirm remaining extensions are expected, updated, and from trusted publishers.",
        })
    elif audit_key == "installed_software":
        profile.update({
            "read": (
                "An installed app or helper has an unsigned, invalid, risky, or weakly corroborated trust state."
            ),
            "likely": (
                "This can be an old app, a manually copied app, a vendor helper, or software installed outside the App Store."
            ),
            "verify": "Verify codesign and notarization, then reinstall from the vendor or remove if unneeded.",
        })
    elif audit_key == "binary_authorization":
        profile.update({
            "read": (
                "Binary-authorization telemetry is missing, unhealthy, or not enforcing. "
                "Runtime anomalies become harder to confirm without it."
            ),
            "likely": (
                "Fresh Santa installs often need system-extension approval, Full Disk Access, or daemon restart."
            ),
            "verify": "Confirm Santa mode, daemon health, system-extension approval, and log generation.",
        })
    elif audit_key in ("persistence", "filesystem_integrity", "sensitive_paths_delta"):
        profile.update({
            "read": (
                "A persistence or sensitive-path signal changed system startup, privileged helper, shell, auth, "
                "or filesystem-integrity state."
            ),
            "likely": (
                "Benign sources include OS updates, Docker/VM tools, security agents, and admin changes. "
                "Unknown signatures, root-owned helpers, or unexplained timestamps need escalation."
            ),
            "verify": "Compare ownership, signature, hash, timestamp, and package ownership against a clean baseline.",
        })
    elif audit_key == "dns":
        profile.update({
            "read": "DNS configuration can redirect or observe traffic metadata and affect every network request.",
            "likely": (
                "Common benign sources are VPNs, Pi-hole/AdGuard, routers, enterprise profiles, or manual DNS settings."
            ),
            "verify": "Confirm each resolver is operator-controlled and re-run DNS posture after changes.",
        })
    elif audit_key in ("kernel_boot", "process_entitlements", "shell_dotfiles"):
        profile.update({
            "read": (
                "The finding affects early boot, runtime hardening, or shell startup. "
                "These areas can amplify later compromise if they are unexpected."
            ),
            "likely": (
                "Developer tooling, audio/storage drivers, virtualization, Homebrew, and browser runtimes commonly "
                "produce noisy but explainable signals."
            ),
            "verify": "Confirm the owner/vendor, compare to expected host role, and remove stale or unknown entries.",
        })
...
def _html_ai_vulnerability_brief(audit_key, finding, expanded=False):
    profile = _ai_vulnerability_profile(audit_key, finding)
    fields = [
        ("AI read", profile["read"]),
        ("Likely cause", profile["likely"]),
        ("Fix", profile["fix"]),
    ]
    if expanded:
        fields.append(("Verify", profile["verify"]))
    body = "".join(
        '<div class="ai-brief-item">'
        f'<span>{_html.escape(label)}</span>'
        f'<p>{_html_with_local_links(value)}</p>'
        '</div>'
        for label, value in fields
    )
    return f'<div class="ai-vuln-brief">{body}</div>'


def _html_remediation_panel(audit_key, finding):
    commands = _remediation_command_plan(audit_key, finding)
    title = (
        f"{finding.get('severity', 'INFO')} - "
        f"{_audit_report_title(audit_key)}"
    )
    finding_text = _html_value(finding.get("message")).strip()
    if _finding_needs_root(audit_key, finding):
        try:
            running_as_root = os.geteuid() == 0
        except AttributeError:
            running_as_root = False
        root_text = (
            "Root required: SecAuditor is already running as root"
            if running_as_root else
            "Root required: sudo commands included"
        )
    else:
        root_text = "Root not inferred"
    return (
        '<button type="button" class="remediate-button" '
        f'data-remediate-title="{_html_attr(title)}" '
        f'data-remediate-finding="{_html_attr(finding_text)}" '
        f'data-remediate-root="{_html_attr(root_text)}" '
        f'data-remediate-commands="{_html_attr(commands)}">'
        "Remediate</button>"
    )


def _html_all_findings_cards(rows):
    if not rows:
        return '<p class="muted">No findings were rendered.</p>'
    rank = {"CRITICAL": 0, "HIGH": 1, "MEDIUM": 2, "INFO": 3, "OK": 4}
    cards = []
    for audit_key, finding, ordinal in rows:
        severity = finding.get("severity", "INFO")
        severity_rank = rank.get(severity, 99)
        audit_title = _audit_report_title(audit_key)
        audit_anchor = "audit-" + _html_anchor(audit_key)
        action = _html_value(finding.get("action")).strip()
        action_html = (
            f'<p class="all-finding-action"><strong>Action:</strong> {_html_with_local_links(action)}</p>'
            f'{_html_remediation_panel(audit_key, finding)}'
            if action else
            '<p class="muted">No remediation action attached.</p>'
        )
        evidence = _html_value(finding.get("evidence")).strip()
        evidence_html = (
            '<details><summary>Evidence</summary>'
        ...
        cards.append(
            f'<article class="all-finding-card row-sev-{_html_severity_class(severity)}" '
            f'data-finding-card data-original-rank="{ordinal}" '
            f'data-severity-rank="{severity_rank}" '
            f'data-audit-title="{_html_attr(audit_title)}">'
            '<div class="all-finding-top">'
            f'<span class="all-finding-index">#{ordinal}</span>'
            f'{_html_badge(severity, severity)}'
            f'<a href="#{audit_anchor}">{_html.escape(audit_title)}</a>'
            '</div>'
            f'<p class="all-finding-message">{_html_with_local_links(finding.get("message", ""))}</p>'
            f'{action_html}'
            f'{evidence_html}'
            '</article>'
        )
    return '<div id="all-findings-list" class="all-findings-list">' + "".join(cards) + "</div>"


def _html_findings_table(rows, include_audit=False):
    if not rows:
        return '<p class="muted">No findings in this section.</p>'
    header_audit = "<th>Audit</th>" if include_audit else ""
    body = []
    for idx, item in enumerate(rows, 1):
        if include_audit:
            audit_key, finding = item
        else:
            audit_key, finding = None, item
        severity = finding.get("severity", "INFO")
        audit_cell = ""
        if include_audit:
            audit_anchor = "audit-" + _html_anchor(audit_key)
            audit_cell = (
                f'<td><a href="#{audit_anchor}">'
                f"{_html.escape(_audit_report_title(audit_key))}</a></td>"
            )
        action_marker = (
            '<span class="mini action-mini">actionable</span>'
            if finding.get("action") else ""
        )
        body.append(
            f'<tr class="row-sev-{_html_severity_class(severity)}">'
            f"<td>{idx}</td>"
            f'<td>{_html_badge(severity, severity)}</td>'
            f"{audit_cell}"
            f"<td>{_html_with_local_links(finding.get('message', ''))} "
            f"{action_marker}</td>"
            f"<td>{_html_finding_detail(finding)}</td>"
            "</tr>"
        )
    return (
        '<table class="findings-table">'
        f"<thead><tr><th>#</th><th>Severity</th>{header_audit}"
        "<th>Finding</th><th>Details</th></tr></thead>"
        f"<tbody>{''.join(body)}</tbody></table>"
    )


def _html_fix_cards(rows, limit=4, empty_message="No CRITICAL/HIGH native findings were found."):
    if not rows:
        return f'<p class="muted">{_html.escape(empty_message)}</p>'
    cards = []
    for audit_key, finding in rows[:limit]:
        severity = finding.get("severity", "INFO")
        audit_anchor = "audit-" + _html_anchor(audit_key)
        cards.append(
            f'<article class="fix-card row-sev-{_html_severity_class(severity)}">'
            f'<div class="fix-card-top">{_html_badge(severity, severity)}'
            f'<a href="#{audit_anchor}">{_html.escape(_audit_report_title(audit_key))}</a></div>'
            f'<p class="fix-message">{_html_with_local_links(finding.get("message", ""))}</p>'
            f'{_html_ai_vulnerability_brief(audit_key, finding)}'
            f'{_html_remediation_panel(audit_key, finding)}'
            '</article>'
        )
    if len(rows) > limit:
        cards.append(
            f'<article class="fix-card more-card"><p>+{len(rows) - limit} more high-priority findings '
            '<a href="#all-findings">in All Findings</a>.</p></article>'
        )
    return '<div class="fix-card-grid">' + "".join(cards) + "</div>"


def _html_reviewer_card(tool, text):
    clean = _clean_report_text(text).strip()
    title = _html.escape(tool.upper())
    if not clean:
        return (
            f'<article class="review-card reviewer-unavailable" id="reviewer-{_html_anchor(tool)}">'
            f"<h3>{title}</h3><p class=\"muted\">No reviewer output.</p></article>"
        )
    if clean.lstrip().startswith("["):
        return (
            f'<article class="review-card reviewer-unavailable" id="reviewer-{_html_anchor(tool)}">'
            f"<h3>{title}</h3><p>{_html.escape(clean)}</p></article>"
        )
    risk = _parse_review_risk(clean) or "reviewed"
    sections = _split_review_sections(clean)
    blocks = [
        (
            f'<article class="review-card reviewer-risk-{_html_severity_class(risk)}" '
            f'id="reviewer-{_html_anchor(tool)}">'
        ),
        f"<h3>{title} {_html_badge(risk, risk)}</h3>",
    ]
    for section in ("EXECUTIVE SUMMARY", "PRIORITY FINDINGS", "INVESTIGATION PLAN"):
        items = sections.get(section, [])[:6]
        if items:
            blocks.append(f"<h4>{_html.escape(section.title())}</h4>")
            blocks.append(_html_list(items, "", ordered=section == "PRIORITY FINDINGS"))
    blocks.append(
        "<details><summary>Raw reviewer output</summary>"
        f"<pre>{_html.escape(clean)}</pre></details></article>"
    )
    return "\n".join(blocks)


def _render_full_audit_html(score, results, reviews, synth_tool,
                            additive_report, text_report):
    """Render the complete automated scan as a navigable HTML report."""
    additive_report = _clean_report_text(additive_report)
    totals = _full_audit_totals(results)
    risk = _parse_review_risk(additive_report)
    if not risk:
    ...
    critical_native = _top_security_findings(results, limit=20, severities=("CRITICAL",))
    all_native = _all_security_findings(results)

    nav_items = [
        ("summary", "Summary"),
        ("critical-remediation", "Critical"),
        ("all-findings", "All Findings"),
        ("audit-heatmap", "Audit Heatmap"),
        ("audit-details", "Audit Details"),
    ]
    generated = time.strftime("%Y-%m-%d %H:%M:%S %Z")
    critical_count = totals["counts"].get("CRITICAL", 0)
    high_count = totals["counts"].get("HIGH", 0)
    primary_takeaway = (
        f"{critical_count} critical findings, {high_count} high findings, "
        f"{totals['actionable']} remediation actions."
    )
...
:root {
  --bg: #02070b;
  --bg-2: #06131d;
  --panel: rgba(8, 20, 30, .82);
  --panel-strong: rgba(10, 31, 43, .94);
  --ink: #e8fbff;
  --muted: #8aa6b2;
  --line: rgba(92, 247, 255, .18);
  --line-hot: rgba(92, 247, 255, .48);
  --accent: #58f7ff;
  --accent-2: #8cffd1;
  --critical: #ff3b5c;
  --high: #ff8a3d;
...
body {
  margin: 0;
  background:
    radial-gradient(circle at 12% 4%, rgba(88,247,255,.22), transparent 24rem),
    radial-gradient(circle at 82% 0%, rgba(65,242,138,.12), transparent 26rem),
    radial-gradient(circle at 50% 100%, rgba(255,59,92,.10), transparent 28rem),
    linear-gradient(135deg, var(--bg) 0%, var(--bg-2) 48%, #020407 100%);
  color: var(--ink);
  font-family: "Avenir Next", "DIN Alternate", "SF Pro Display", ui-sans-serif, system-ui, sans-serif;
  min-height: 100vh;
...
body::before {
  ...
  pointer-events: none;
  background-image:
    linear-gradient(rgba(88,247,255,.06) 1px, transparent 1px),
    linear-gradient(90deg, rgba(88,247,255,.05) 1px, transparent 1px);
...
::selection { background: rgba(88,247,255,.35); color: white; }
a {
  color: var(--accent);
  text-decoration: none;
  border-bottom: 1px solid rgba(88,247,255,.36);
}
a:hover {
  color: var(--accent-2);
  border-bottom-color: var(--accent-2);
  text-shadow: 0 0 12px rgba(140,255,209,.45);
}
.layout { display: grid; grid-template-columns: 20.5rem minmax(0, 1fr); min-height: 100vh; position: relative; }
.toc {
  position: sticky; top: 0; height: 100vh; overflow: auto;
  padding: 1.25rem; border-right: 1px solid var(--line);
  background:
    linear-gradient(180deg, rgba(9,24,35,.96), rgba(3,10,15,.92)),
    radial-gradient(circle at 50% 0%, rgba(88,247,255,.15), transparent 14rem);
  backdrop-filter: blur(16px);
  box-shadow: 12px 0 45px rgba(0,0,0,.35);
}
.toc h1 {
  font-family: ui-monospace, SFMono-Regular, Menlo, monospace;
  font-size: 1rem; letter-spacing: .20em; text-transform: uppercase;
  margin: 0 0 .9rem; color: var(--accent);
  text-shadow: 0 0 18px rgba(88,247,255,.55);
}
.toc h1::before { content: "SEC::"; color: var(--accent-2); }
.toc .small {
  padding: .75rem;
  border: 1px solid rgba(88,247,255,.12);
  border-radius: 14px;
  background: rgba(88,247,255,.035);
}
.toc .small strong, strong {
  color: var(--accent);
  text-shadow: 0 0 12px rgba(88,247,255,.25);
}
.toc a {
  display: block;
  color: var(--ink);
...
.toc a:hover {
  background: rgba(88,247,255,.08);
  border-color: rgba(88,247,255,.24);
}
.toc .small {
  color: var(--muted); font-size: .78rem; line-height: 1.45;
  font-family: ui-monospace, SFMono-Regular, Menlo, monospace;
}
main { padding: 2.35rem; max-width: 96rem; }
.hero, section, .review-card {
  position: relative;
  overflow: hidden;
  background:
    linear-gradient(145deg, rgba(12,31,44,.92), rgba(5,14,22,.86)),
    radial-gradient(circle at 100% 0%, rgba(88,247,255,.13), transparent 22rem);
  border: 1px solid var(--line);
  border-radius: 22px;
  box-shadow:
    0 24px 80px rgba(0,0,0,.42),
    inset 0 1px 0 rgba(255,255,255,.04),
    0 0 0 1px rgba(88,247,255,.04);
  margin-bottom: 1.25rem;
  padding: 1.45rem;
}
.hero::before, section::before, .review-card::before {
  content: "";
  position: absolute;
  inset: 0 0 auto 0;
  height: 2px;
...
.hero {
  ...
  background:
    linear-gradient(135deg, rgba(8,20,31,.96), rgba(4,9,15,.9)),
    radial-gradient(circle at 75% 18%, rgba(88,247,255,.24), transparent 20rem),
    radial-gradient(circle at 14% 90%, rgba(65,242,138,.16), transparent 18rem);
  padding: clamp(1.15rem, 2vw, 1.85rem);
}
.hero::after {
  content: "THREAT MATRIX";
  position: absolute;
  right: 1.2rem;
  top: 1rem;
  color: rgba(88,247,255,.10);
  font: 900 clamp(2.5rem, 8vw, 7rem) ui-monospace, SFMono-Regular, Menlo, monospace;
  letter-spacing: -.08em;
  pointer-events: none;
}
.hero h2 {
  ...
  margin: 0 0 .75rem;
  max-width: 11ch;
  text-shadow: 0 0 26px rgba(88,247,255,.22);
  color: #f3feff;
}
.hero h2::before { content: ""; }
.hero-kicker {
  display: flex;
  flex-wrap: wrap;
  gap: .55rem;
  margin-bottom: 1rem;
  color: var(--accent);
  font: 900 .76rem ui-monospace, SFMono-Regular, Menlo, monospace;
  letter-spacing: .12em;
  text-transform: uppercase;
}
.hero-kicker span {
  border: 1px solid rgba(88,247,255,.22);
  border-radius: 999px;
  padding: .32rem .58rem;
  background: rgba(88,247,255,.055);
}
.hero-grid {
  position: relative;
  z-index: 1;
...
.hero-badges { display: flex; flex-wrap: wrap; gap: .55rem; margin: 1rem 0 1.25rem; }
.lede {
  max-width: 68ch;
  margin: 0;
  color: #dbf8fc;
...
.score-orb {
  ...
  position: relative;
  background:
    radial-gradient(circle at 50% 50%, #061520 0 49%, transparent 50%),
    conic-gradient(var(--accent) calc(var(--score) * 1%), rgba(88,247,255,.10) 0);
  box-shadow:
    0 0 45px rgba(88,247,255,.18),
    inset 0 0 36px rgba(88,247,255,.09);
}
.score-orb::before {
  content: "";
  position: absolute;
  inset: 10%;
  border-radius: 50%;
  border: 1px solid rgba(88,247,255,.24);
  background:
    radial-gradient(circle, rgba(88,247,255,.12), transparent 64%),
    repeating-conic-gradient(from 0deg, rgba(88,247,255,.22) 0 2deg, transparent 2deg 12deg);
  mask: radial-gradient(circle, transparent 0 59%, black 60%);
}
.score-orb::after {
  content: "";
  position: absolute;
  inset: 18%;
  border-radius: 50%;
  background: linear-gradient(180deg, rgba(10,31,43,.96), rgba(2,8,12,.98));
  border: 1px solid rgba(88,247,255,.2);
}
.score-inner {
  position: relative;
  z-index: 1;
...
.score-inner .score-number {
  ...
  letter-spacing: -.08em;
  color: #f5feff;
  text-shadow: 0 0 28px rgba(88,247,255,.28);
}
.score-inner .score-label {
  display: block;
  margin-top: .55rem;
  color: var(--accent);
  font: 900 .78rem ui-monospace, SFMono-Regular, Menlo, monospace;
  letter-spacing: .12em;
  text-transform: uppercase;
}
h2, h3 {
  color: #effcff;
  text-shadow: 0 0 18px rgba(88,247,255,.16);
}
h2 {
  font-size: 1.55rem;
...
h2::before {
  ...
  color: var(--accent);
  font-family: ui-monospace, SFMono-Regular, Menlo, monospace;
  text-shadow: 0 0 16px rgba(88,247,255,.5);
}
h3 { margin: 0 0 .75rem; font-size: 1.18rem; }
h3::before {
  content: "◇ ";
  color: var(--accent-2);
  text-shadow: 0 0 14px rgba(140,255,209,.35);
}
h4 {
  margin: 1rem 0 .35rem;
  color: var(--accent);
...
.metric-grid, .layer-grid, .review-grid {
  display: grid; gap: .85rem; grid-template-columns: repeat(auto-fit, minmax(12rem, 1fr));
}
.metric-grid.hero-metrics { grid-template-columns: repeat(3, minmax(10rem, 1fr)); margin-top: 1.1rem; }
.score-card {
  border-color: rgba(88,247,255,.46);
  background:
    linear-gradient(135deg, rgba(9,27,40,.96), rgba(3,9,14,.92)),
    radial-gradient(circle at 12% 12%, rgba(88,247,255,.22), transparent 22rem),
    radial-gradient(circle at 84% 0%, rgba(140,255,209,.14), transparent 20rem);
}
.score-card-grid {
  display: grid;
...
.score-face {
  ...
  padding: 1.15rem;
  background:
    linear-gradient(180deg, rgba(88,247,255,.09), rgba(255,255,255,.025)),
    radial-gradient(circle at 100% 0%, rgba(88,247,255,.20), transparent 14rem);
}
.score-face::after {
  content: "SCORE";
  position: absolute;
  right: -.35rem;
  bottom: -.65rem;
  color: rgba(88,247,255,.08);
  font: 1000 5.8rem ui-monospace, SFMono-Regular, Menlo, monospace;
  letter-spacing: -.08em;
}
.score-title {
  color: var(--accent);
...
.score-number-compact {
  ...
  line-height: .82;
  letter-spacing: -.08em;
  text-shadow: 0 0 30px rgba(88,247,255,.28);
}
.score-number-compact span {
  color: var(--accent);
  font-size: .26em;
  letter-spacing: -.03em;
}
...
.metric, .layer-card {
  ...
  border-radius: 16px;
  padding: .95rem;
  background: linear-gradient(180deg, rgba(88,247,255,.07), rgba(255,255,255,.025));
...
.metric .label, .layer-card .label {
  color: var(--accent);
  font: 800 .72rem ui-monospace, SFMono-Regular, Menlo, monospace;
  text-transform: uppercase;
  letter-spacing: .09em;
}
.metric .value, .layer-card .value {
  display: block;
  font-size: 2rem;
  font-weight: 900;
  margin-top: .18rem;
  letter-spacing: -.04em;
  color: #f7feff;
  text-shadow: 0 0 20px rgba(88,247,255,.18);
}
.layer-card {
  overflow: hidden;
}
.layer-bar {
  ...
  border-radius: 999px;
  margin-top: .8rem;
  background: rgba(88,247,255,.10);
  overflow: hidden;
  border: 1px solid rgba(88,247,255,.14);
}
.layer-bar span {
  display: block;
  height: 100%;
  ...
  box-shadow: 0 0 18px rgba(88,247,255,.28);
}
.badge, .mini {
  display: inline-flex; align-items: center; gap: .25rem;
  border-radius: 999px;
  padding: .22rem .62rem;
  font: 900 .72rem ui-monospace, SFMono-Regular, Menlo, monospace;
  color: #061014;
  white-space: nowrap;
  letter-spacing: .04em;
  box-shadow: 0 0 18px rgba(88,247,255,.12);
}
.sev-critical { background: var(--critical); color: white; box-shadow: 0 0 18px rgba(255,59,92,.38); }
.sev-high { background: var(--high); color: #160a02; box-shadow: 0 0 18px rgba(255,138,61,.30); }
.sev-medium { background: var(--medium); color: #171000; box-shadow: 0 0 18px rgba(255,209,102,.25); }
.sev-info { background: var(--info); color: #03101f; box-shadow: 0 0 18px rgba(82,168,255,.28); }
.sev-ok, .sev-low { background: var(--ok); color: #021107; box-shadow: 0 0 18px rgba(65,242,138,.28); }
.mini { background: rgba(140,255,209,.18); color: var(--accent-2); border: 1px solid rgba(140,255,209,.28); font-size: .63rem; margin-left: .35rem; }
.action-pill {
...
.severity-vector .seg { height: 100%; min-width: 2%; }
.sev-bg-critical { background: var(--critical); box-shadow: 0 0 16px rgba(255,59,92,.35); }
.sev-bg-high { background: var(--high); }
.sev-bg-medium { background: var(--medium); }
.sev-bg-info { background: var(--info); }
.sev-bg-ok { background: var(--ok); }
.critical-remediation {
  border-color: rgba(140,255,209,.28);
  background:
    linear-gradient(145deg, rgba(8,28,35,.94), rgba(4,12,18,.90)),
    radial-gradient(circle at 0% 0%, rgba(140,255,209,.12), transparent 24rem);
}
.critical-remediation .fix-card-grid {
...
.finding-toolbar {
  ...
  border: 1px solid rgba(88,247,255,.14);
  border-radius: 16px;
  background: rgba(1,7,11,.30);
}
.finding-toolbar .toolbar-label {
  color: var(--muted);
  font: 900 .72rem ui-monospace, SFMono-Regular, Menlo, monospace;
  letter-spacing: .08em;
  text-transform: uppercase;
...
.sort-button.active {
  color: var(--accent-2);
  border-color: rgba(140,255,209,.58);
  background: linear-gradient(90deg, rgba(88,247,255,.18), rgba(140,255,209,.22));
}
.all-findings-list {
  display: grid;
  gap: .72rem;
...
.all-finding-card {
  ...
  border-radius: 16px;
  padding: .85rem;
  background: rgba(1,7,11,.32);
  overflow-wrap: anywhere;
}
.all-finding-card.row-sev-critical { border-color: rgba(255,59,92,.34); }
.all-finding-card.row-sev-high { border-color: rgba(255,138,61,.30); }
.all-finding-card.row-sev-medium { border-color: rgba(255,209,102,.24); }
.all-finding-top {
  display: flex;
  flex-wrap: wrap;
  align-items: center;
...
.human-card, .fix-card {
  ...
  border-radius: 18px;
  padding: 1rem;
  background: rgba(1,7,11,.34);
  box-shadow: inset 0 1px 0 rgba(255,255,255,.04);
}
.human-card h3, .fix-card h3 { margin-bottom: .55rem; }
.human-card p { margin-top: 0; line-height: 1.5; }
.human-card ul, .human-card ol { margin-bottom: 0; }
.fix-card-grid { display: grid; gap: .72rem; }
.fix-card {
  position: relative;
  overflow: hidden;
  overflow-wrap: anywhere;
}
.fix-card.row-sev-critical { border-color: rgba(255,59,92,.35); }
.fix-card.row-sev-high { border-color: rgba(255,138,61,.32); }
.fix-card-top {
  display: flex;
  align-items: center;
...
.ai-brief-item {
  ...
  border-radius: 14px;
  padding: .68rem .75rem;
  background: linear-gradient(180deg, rgba(88,247,255,.055), rgba(255,255,255,.022));
}
.ai-brief-item span {
  display: block;
  margin-bottom: .25rem;
...
.remediation-panel {
  margin-top: .9rem;
  border-top: 1px solid rgba(88,247,255,.14);
  padding-top: .75rem;
}
...
.remediate-button, .copy-button {
  display: inline-flex;
  align-items: center;
  justify-content: center;
  gap: .4rem;
  border: 1px solid rgba(88,247,255,.34);
  border-radius: 999px;
  background: linear-gradient(90deg, rgba(88,247,255,.16), rgba(140,255,209,.13));
  color: var(--accent);
  cursor: pointer;
  font: 900 .72rem ui-monospace, SFMono-Regular, Menlo, monospace;
  letter-spacing: .07em;
  padding: .48rem .72rem;
  text-transform: uppercase;
  box-shadow: 0 0 18px rgba(88,247,255,.10);
}
.remediate-button::before { content: "↳"; color: var(--accent-2); }
.remediate-button:hover, .copy-button:hover {
  border-color: rgba(140,255,209,.56);
  color: var(--accent-2);
  box-shadow: 0 0 22px rgba(88,247,255,.18);
}
.copy-button {
  margin-top: .45rem;
  padding: .38rem .62rem;
  background: rgba(1,7,11,.42);
}
.copy-button.copied {
  color: var(--ok);
  border-color: rgba(65,242,138,.55);
}
.remediation-modal[hidden] { display: none; }
.remediation-modal {
  position: fixed;
  inset: 0;
  z-index: 50;
...
.remediation-backdrop {
  ...
  inset: 0;
  background:
    radial-gradient(circle at 50% 0%, rgba(88,247,255,.20), transparent 28rem),
    rgba(0, 5, 9, .78);
...
.remediation-dialog {
  ...
  overflow: auto;
  border-color: rgba(88,247,255,.48);
  box-shadow:
    0 28px 100px rgba(0,0,0,.72),
    0 0 44px rgba(88,247,255,.16);
}
.modal-close {
  position: absolute;
  top: 1rem;
  ...
  background:
    ...
    rgba(255,59,92,.18);
  border-color: rgba(255,59,92,.46);
}
.modal-close:hover {
  color: white;
  border-color: rgba(255,59,92,.72);
  box-shadow: 0 0 24px rgba(255,59,92,.24);
}
.modal-kicker {
  display: flex;
  flex-wrap: wrap;
...
.remediation-dialog pre {
  max-height: 48vh;
  margin: 0;
  border-radius: 0 0 18px 18px;
  border-color: rgba(88,247,255,.26);
}
.command-panel {
  margin-top: 1rem;
  overflow: hidden;
  border: 1px solid rgba(88,247,255,.18);
  border-radius: 18px;
  background: rgba(0,6,10,.64);
  box-shadow: inset 0 0 38px rgba(88,247,255,.04);
}
.command-toolbar {
  display: flex;
  align-items: center;
  ...
  gap: .85rem;
  padding: .72rem .82rem;
  border-bottom: 1px solid rgba(88,247,255,.16);
  background:
    linear-gradient(90deg, rgba(88,247,255,.12), rgba(140,255,209,.06)),
    rgba(1,7,11,.68);
}
...
.modal-copy {
  ...
  height: 2.35rem;
  padding: 0;
  border-color: rgba(140,255,209,.46);
  color: var(--accent-2);
  background: linear-gradient(90deg, rgba(88,247,255,.16), rgba(140,255,209,.20));
}
.modal-copy .copy-icon {
  display: block;
  filter: drop-shadow(0 0 8px rgba(140,255,209,.25));
}
.modal-copy.copied .copy-icon {
  color: var(--ok);
}
.sr-only {
  position: absolute;
...
.remediation-option h4 { margin-top: 0; }
.remediation-option p { margin: .25rem 0; }
.remediation-option code {
  color: var(--accent-2);
  font-family: ui-monospace, SFMono-Regular, Menlo, monospace;
  overflow-wrap: anywhere;
}
.ai-remediation {
  grid-column: 1 / -1;
  border-color: rgba(88,247,255,.20);
}
.ai-remediation .ai-vuln-brief {
  grid-template-columns: repeat(2, minmax(0, 1fr));
}
.ai-option {
  grid-column: 1 / -1;
  border-color: rgba(140,255,209,.22);
}
.ai-option pre {
  max-height: 18rem;
  margin-bottom: 0;
...
table {
  ...
  border: 1px solid rgba(88,247,255,.12);
  border-radius: 14px;
  background: rgba(1,7,11,.38);
}
tr:nth-child(even) td { background: rgba(88,247,255,.025); }
tr:hover td { background: rgba(88,247,255,.06); }
tr.row-sev-critical td:first-child { box-shadow: inset 4px 0 0 var(--critical); }
tr.row-sev-high td:first-child { box-shadow: inset 4px 0 0 var(--high); }
tr.row-sev-medium td:first-child { box-shadow: inset 4px 0 0 var(--medium); }
tr.row-sev-info td:first-child { box-shadow: inset 4px 0 0 var(--info); }
tr.row-sev-ok td:first-child { box-shadow: inset 4px 0 0 var(--ok); }
th, td { border-top: 1px solid rgba(88,247,255,.12); padding: .72rem; text-align: left; vertical-align: top; }
td:first-child {
  color: var(--accent);
  font-family: ui-monospace, SFMono-Regular, Menlo, monospace;
}
...
pre {
  ...
  border-radius: 14px;
  padding: .9rem;
  box-shadow: inset 0 0 30px rgba(88,247,255,.04);
}
...
li { margin-bottom: .45rem; color: #d8f4f8; }
li::marker { color: var(--accent); }
.audit-list { columns: 1; list-style: none; padding-left: 0; }
.audit-list li {
  break-inside: avoid;
  margin-bottom: .32rem;
  padding: .45rem .55rem;
  border: 1px solid rgba(88,247,255,.10);
  border-radius: 12px;
  background: rgba(255,255,255,.025);
}
.audit-list .muted { display: block; margin-top: .18rem; font-size: .72rem; }
.path-link {
  font-family: ui-monospace, SFMono-Regular, Menlo, monospace;
  color: var(--accent-2);
  border-bottom-color: rgba(140,255,209,.36);
}
.review-card.reviewer-unavailable { opacity: .72; filter: saturate(.65); }
.review-card.reviewer-risk-critical { border-color: rgba(255,59,92,.42); }
.review-card.reviewer-risk-high { border-color: rgba(255,138,61,.35); }
.review-card.reviewer-risk-medium { border-color: rgba(255,209,102,.30); }
@media (max-width: 840px) {
  .layout { display: block; }
  .toc { position: static; height: auto; }
  main { padding: 1rem; }
  .hero { min-height: 17rem; }
  .hero-grid { grid-template-columns: 1fr; }
  .score-orb { width: min(14rem, 90vw); }
  .score-card-grid { grid-template-columns: 1fr; }
  .metric-grid.hero-metrics { grid-template-columns: 1fr; }
  .brief-grid { grid-template-columns: 1fr; }
  .ai-remediation .ai-vuln-brief { grid-template-columns: 1fr; }
  .remediation-grid { grid-template-columns: 1fr; }
  table { display: block; overflow-x: auto; }
}
"""
    nav_html = "\n".join(
        f'<a href="#{anchor}">{_html.escape(label)}</a>'
        for anchor, label in nav_items
    )
    audit_nav = "\n".join(
        f'<li><a href="#audit-{_html_anchor(key)}">{_html.escape(_audit_report_title(key))}</a> '
        f'<span class="muted">({_html.escape(_format_severity_counts(_counts_for_findings(findings)))})</span></li>'
        for key, findings in results.items()
    )
    layer_cards = []
    for layer in ("Network", "System", "Kernel", "Library"):
        value = score["layers"].get(layer, 0)
        layer_band = _severity_band(value)
        layer_cards.append(
            f'<div class="layer-card" style="--score:{max(0, min(100, int(value)))}">'
            f'<span class="label">{_html.escape(layer)}</span>'
            f'<span class="value">{value}/100</span>'
            f'{_html_badge(layer_band, _band_to_severity(layer_band))}'
            '<div class="layer-bar"><span></span></div>'
            "</div>"
        )
    heatmap_rows = []
    for key, findings in results.items():
        ...
        heatmap_rows.append(
            "<tr>"
            f'<td><a href="#audit-{_html_anchor(key)}">{_html.escape(_audit_report_title(key))}</a></td>'
            f"<td>{_html.escape(_LAYER_FOR_AUDIT.get(key, 'Library'))}</td>"
            f'<td><div class="counts">{_html_counts_badges(counts)}</div>{_html_severity_vector(counts)}</td>'
    ...
    for key, findings in results.items():
        ...
        audit_sections.append(
            f'<section id="audit-{_html_anchor(key)}">'
            f"<h3>{_html.escape(_audit_report_title(key))}</h3>"
            f'<p class="counts">{_html_counts_badges(counts)}</p>'
            f"{_html_findings_table(findings)}"
            "</section>"
        )
...
<title>SecAuditor security scan - {_html.escape(risk)}</title>
<style>{css}</style>
</head>
<body>
<div class="layout">
<aside class="toc">
  <h1>secprocmon</h1>
  <p class="small">Automated security scan<br>Generated {_html.escape(generated)}</p>
  {nav_html}
  <h2 class="small">Audits</h2>
  <ul class="audit-list">{audit_nav}</ul>
</aside>
<main>
<section class="score-card" id="summary">
  <div class="score-card-grid">
    <div class="score-face">
      <span class="score-title">Security score</span>
      <span class="score-number-compact">{score['global']}<span>/100</span></span>
      <div class="hero-badges">{_html_badge(risk, risk)} {_html_badge(band, _band_to_severity(band))}</div>
      <p class="lede">{_html_with_local_links(primary_takeaway)}</p>
    </div>
    <div class="score-card-side">
      <div class="metric-grid hero-metrics">
        <div class="metric"><span class="label">Reviewed risk</span><span class="value">{_html.escape(risk)}</span></div>
        <div class="metric"><span class="label">Findings</span><span class="value">{totals['total_findings']}</span></div>
        <div class="metric"><span class="label">Actionable</span><span class="value">{totals['actionable']}</span></div>
        <div class="metric"><span class="label">Critical</span><span class="value">{critical_count}</span></div>
        <div class="metric"><span class="label">High</span><span class="value">{high_count}</span></div>
        <div class="metric"><span class="label">Reviewers</span><span class="value">{len(available_reviewers)}</span></div>
      </div>
      <div class="severity-panel">
        {_html_severity_vector(totals["counts"])}
        <div class="counts">{_html_counts_badges(totals["counts"])}</div>
      </div>
...
<section id="critical-remediation" class="critical-remediation">
  <h2>Critical Findings & Remediation</h2>
  {_html_fix_cards(critical_native, limit=20, empty_message="No CRITICAL findings were found.")}
</section>

<section id="all-findings">
  <h2>All Findings</h2>
  <div class="finding-toolbar" aria-label="Finding sort controls">
    <span class="toolbar-label">{len(all_native)} findings</span>
    <button type="button" class="copy-button sort-button active" data-sort-findings="original">Original</button>
    <button type="button" class="copy-button sort-button" data-sort-findings="criticality">Criticality</button>
    <button type="button" class="copy-button sort-button" data-sort-findings="audit">Audit</button>
  </div>
  {_html_all_findings_cards(all_native)}
</section>

<section id="audit-heatmap">
  <h2>Per-Audit Heatmap</h2>
  <table><thead><tr><th>Audit</th><th>Layer</th><th>Severity</th><th>Total</th></tr></thead><tbody>{''.join(heatmap_rows)}</tbody></table>
</section>
...
<div class="remediation-modal" id="remediation-modal" hidden>
  <div class="remediation-backdrop" data-close-remediation></div>
  <section class="remediation-dialog" role="dialog" aria-modal="true" aria-labelledby="remediation-modal-title">
    <button type="button" class="copy-button modal-close" data-close-remediation aria-label="Dismiss remediation dialog">X</button>
    <div class="modal-kicker">
      <span id="remediation-modal-root">Root status</span>
      <span>Verification included</span>
    </div>
    <h2 id="remediation-modal-title">Remediation commands</h2>
    <p id="remediation-modal-finding" class="lede"></p>
    <div class="command-panel">
      <div class="command-toolbar">
        <span>Command Plan</span>
        <button type="button" class="copy-button modal-copy" id="remediation-copy"
                data-copy="" data-copy-feedback="icon"
                aria-label="Copy commands" title="Copy commands">
          {_html_copy_icon()}
        </button>
      </div>
      <pre id="remediation-modal-commands"></pre>
    </div>
    <p class="safety-note">Safety: this browser report does not execute remediation commands. Review the commands, preserve evidence first, then run them from a terminal.</p>
  </section>
</div>
<script>
document.addEventListener("click", async (event) => {{
  const sortButton = event.target.closest("[data-sort-findings]");
  if (sortButton) {{
    const list = document.getElementById("all-findings-list");
    if (!list) return;
    const mode = sortButton.getAttribute("data-sort-findings") || "original";
    const cards = Array.from(list.querySelectorAll("[data-finding-card]"));
    cards.sort((a, b) => {{
      if (mode === "criticality") {{
        const severityDelta = Number(a.dataset.severityRank || 99) - Number(b.dataset.severityRank || 99);
        if (severityDelta) return severityDelta;
      }} else if (mode === "audit") {{
        const auditDelta = (a.dataset.auditTitle || "").localeCompare(b.dataset.auditTitle || "");
        if (auditDelta) return auditDelta;
      }}
      return Number(a.dataset.originalRank || 0) - Number(b.dataset.originalRank || 0);
    }});
    cards.forEach((card) => list.appendChild(card));
    document.querySelectorAll("[data-sort-findings]").forEach((button) => {{
      button.classList.toggle("active", button === sortButton);
    }});
    return;
  }}
  const remediator = event.target.closest("[data-remediate-commands]");
  if (remediator) {{
    const modal = document.getElementById("remediation-modal");
    const title = document.getElementById("remediation-modal-title");
    const finding = document.getElementById("remediation-modal-finding");
    const root = document.getElementById("remediation-modal-root");
    const commands = document.getElementById("remediation-modal-commands");
    const copy = document.getElementById("remediation-copy");
    const commandText = remediator.getAttribute("data-remediate-commands") || "";
    title.textContent = remediator.getAttribute("data-remediate-title") || "Remediation commands";
    finding.textContent = remediator.getAttribute("data-remediate-finding") || "";
    root.textContent = remediator.getAttribute("data-remediate-root") || "Root status unknown";
    commands.textContent = commandText;
    copy.setAttribute("data-copy", commandText);
    modal.hidden = false;
    document.body.classList.add("modal-open");
    copy.focus();
    return;
  }}
  if (event.target.closest("[data-close-remediation]")) {{
    const modal = document.getElementById("remediation-modal");
  ...
  try {{
    ...
    const iconFeedback = button.getAttribute("data-copy-feedback") === "icon";
    ...
    setTimeout(() => {{
      if (iconFeedback) {{
        ...
        button.setAttribute("title", originalLabel || "Copy");
      }} else {{
...
def _write_html_report(html_report, directory=None):
    """Write an HTML scan report and return the absolute file path."""
    target_dir = os.path.abspath(os.path.expanduser(directory or _HTML_REPORT_DIR))
    os.makedirs(target_dir, exist_ok=True)
    stamp = time.strftime("%Y%m%d-%H%M%S")
    path = os.path.join(
        target_dir,
        f"secauditor-security-scan-{stamp}-{os.getpid()}.html",
    )
...
def _open_html_report(path, timeout=10):
    """Open an HTML report in the system browser/default handler."""
    ...
    if sys.platform == "darwin" and shutil.which("open"):
        try:
            ...
            return False, str(e)
        if proc.returncode == 0:
            return True, "opened via macOS open"
        detail = (proc.stderr or proc.stdout or f"open exited {proc.returncode}").strip()
        return False, detail
    try:
        import webbrowser as _webbrowser
        url = "file://" + _urlparse.quote(abs_path)
        return bool(_webbrowser.open(url)), "opened via webbrowser"
    except Exception as e:
        return False, str(e)


def _headless_progress(message, stream=None):
    """Emit timestamped progress for long headless scans."""
    if stream is None:
        stream = sys.stderr
    with _HEADLESS_PROGRESS_LOCK:
        print(f"[secprocmon] {time.strftime('%H:%M:%S')} {message}",
              file=stream, flush=True)


def _render_full_audit_report(progress=None, html_report=False,
                              open_html=False, html_dir=None):
    """Run the full audit suite and return an additive scan report."""
    if progress:
        progress("automated security scan: starting full audit suite")
    results = _full_audit_results(progress=progress)
    if progress:
        progress("automated security scan: scoring findings")
    score = _score_findings(results)
    if progress:
        progress("automated security scan: building reviewer corpus")
    llm_body = _build_full_audit_llm_body(results, score)
    if progress:
        progress("automated security scan: starting Claude/Codex/Gemini reviews")
    reviews = _run_llm_reviews_parallel(
        _FULL_AUDIT_REVIEW_PROMPT,
        llm_body,
    ...
    if progress:
        progress("automated security scan: synthesizing additive report")
    synth_tool, additive_report = _synthesize_full_audit_reviews(
        score, results, llm_body, reviews, progress=progress)
    if progress:
        progress("automated security scan: rendering final report")
    text_report = _compose_full_audit_report(
        score, results, reviews, synth_tool, additive_report)
    if not html_report:
        return text_report

    if progress:
        progress("automated security scan: writing HTML report")
    try:
        rendered_html = _render_full_audit_html(
            score, results, reviews, synth_tool, additive_report, text_report)
        html_path = _write_html_report(rendered_html, directory=html_dir)
    except Exception as e:
        if progress:
            progress(f"automated security scan: HTML report error {e}")
        return text_report + f"\n\nHTML REPORT\n- HTML report failed: {e}"

    if progress:
        progress(f"automated security scan: HTML report written {html_path}")
    open_status = "not requested"
    if open_html:
        if progress:
            progress("automated security scan: opening HTML report in browser")
        opened, detail = _open_html_report(html_path)
        if opened:
            open_status = "opened in browser"
            if progress:
                progress("automated security scan: HTML report opened in browser")
        else:
            open_status = f"open failed: {detail}"
            if progress:
                progress(f"automated security scan: HTML report open failed: {detail}")

    html_note = (
        f"HTML report: {html_path}\n"
        f"Browser: {open_status}"
    )
    marker = "READ THIS FIRST\n"
    if marker in text_report:
        return text_report.replace(marker, marker + html_note + "\n", 1)
    return text_report + "\n\nHTML REPORT\n" + html_note
...
class ProcMonUI:
    def __init__(self, stdscr, name, interval, skip_fd):
        ...
        self._test_mode = os.environ.get("SECPROCMON_TEST_MODE", "").lower() not in (
            "", "0", "false", "no")
        self._tui_capture_dir = os.environ.get("SECPROCMON_TUI_CAPTURE_DIR", "").strip()
        self._tui_capture_action = os.environ.get("SECPROCMON_TUI_CAPTURE_ACTION", "").strip()
        self._test_start_action = os.environ.get("SECPROCMON_TEST_ACTION", "").strip()
        self._test_select_pid = int(os.environ.get("SECPROCMON_TEST_SELECT_PID", "0") or 0)
    ...
    def _test_detail_metadata(self, title):
        ...
        if self._audit_mode:
            findings = None if self._audit_type == "automated_security_scan" else self._audit_findings_structured
            return ("audit", self._audit_type or "unknown", title,
                    self._audit_loading, findings)
        if self._keyscan_mode:
            return ("forensic", "keyscan", title,
                    self._keyscan_loading, self._keyscan_findings_structured)
    ...
    def _detail_ready_state(self):
        ...
        if self._audit_mode:
            ...
            ready = bool(self._keyscan_lines) and not self._keyscan_loading
    ...
    def _start_llm_summary(self, scope, title, findings):
        ...
        if (getattr(self, "_test_mode", False)
                and os.environ.get("SECPROCMON_TEST_ALLOW_LLM", "").lower()
    ...
    _AUDIT_SCANS = {
        "process_triage": (None,
                            "Deep Process Triage"),
        "secauditor_bridge": (None,
                              "SecAuditor Browser/API"),
        "automated_security_scan": (None,
                                    "Automated Security Scan"),
        "network": (_audit_network_exposure,
        ...
        "binary_authorization": (_audit_binary_authorization,
                                 "Binary Authorization Telemetry"),
        "tool_correlation": (_audit_tool_correlation,
                              "Optional Backend Correlation"),
        "packages": (_audit_package_managers,
    ...
    def _test_automated_security_scan_report(self):
        """Build a deterministic automated scan report for TUI regression."""
        results = {
            "network": [{
                "severity": "OK",
                "message": "Test mode network posture clean",
                "action": None,
            }],
            "injection_antidebug": [{
                "severity": "HIGH",
                "message": "Test mode DYLD injection marker requires investigation",
                "evidence": "DYLD_INSERT_LIBRARIES=/tmp/libinject.dylib",
                "action": {"type": "kill_process", "pid": 4242, "exe": "/tmp/suspicious"},
            }],
            "tool_correlation": [{
                "severity": "MEDIUM",
                "message": "Test mode Santa and osquery corroboration present",
                "action": None,
            }],
        }
        score = _score_findings(results)
        reviews = {
            "claude": (
                "RISK: HIGH\n"
                "EXECUTIVE SUMMARY:\n"
                "- Claude review: injection marker is the top test signal.\n"
                "PRIORITY FINDINGS:\n"
                "- HIGH injection marker deserves immediate triage.\n"
                "TOOL-CORRELATED SIGNALS:\n"
                "- Santa and osquery corroborate the runtime signal.\n"
                "INVESTIGATION PLAN:\n"
                "- Inspect PID 4242 and verify the injected library.\n"
                "FALSE-POSITIVE NOTES:\n"
                "- Developer test harnesses can set DYLD variables.\n"
            ),
            "codex": (
                "RISK: HIGH\n"
                "EXECUTIVE SUMMARY:\n"
                "- Codex review agrees the additive report is actionable.\n"
                "PRIORITY FINDINGS:\n"
                "- HIGH live injection signal is supported by test evidence.\n"
                "TOOL-CORRELATED SIGNALS:\n"
                "- Optional backends are corroboration, not standalone proof.\n"
                "INVESTIGATION PLAN:\n"
                "- Preserve evidence before remediation.\n"
                "FALSE-POSITIVE NOTES:\n"
                "- Controlled lab processes can look suspicious.\n"
            ),
            "gemini": "[gemini CLI not found]",
        }
        additive_report = _local_additive_full_audit_review(
            score, results, reviews)
        return _compose_full_audit_report(
            score, results, reviews, None, additive_report)

    def _format_automated_security_scan_report(self):
        """Return display lines for the in-app additive full-host scan."""
        self._audit_findings_structured = []
        self._audit_line_for_finding = []
        if (getattr(self, "_test_mode", False)
                and os.environ.get("SECPROCMON_TEST_ALLOW_LLM", "").lower()
            ...
            self._audit_progress(
                "automated security scan: building deterministic test report")
            report = self._test_automated_security_scan_report()
        else:
            report = _render_full_audit_report(
                progress=self._audit_progress,
                html_report=True,
                open_html=True,
            )
    ...
    def _eslogger_select_prefixes(self):
        ...
        raw = os.environ.get("SECPROCMON_ES_SELECT_PREFIXES", "").strip()
        if not raw:
            return []
    ...
    def _format_traffic_view(self):
        ...
        if self._traffic_error:
            ...
            if "not found" in self._traffic_error:
                ...
                lines.append("    1. Start secprocmon again and open Traffic Inspector")
    ...
    def _build_process_triage_findings(self, pid, command, triage_context=None):
        ...
        vt = artifacts.get("virustotal") or {}
        if vt.get("found") and (vt.get("malicious", 0) or vt.get("suspicious", 0)):
            sev = "CRITICAL" if vt.get("malicious", 0) else "HIGH"
            findings.append({
                "severity": sev,
                "message": "VirusTotal reputation is negative for the selected binary",
                "evidence": (
    ...
    def _collect_chat_context(self):
        ...
        if self._inspect_mode and self._inspect_lines:
            ...
            parts.append(
                ...
                "Full output:")
            parts.append(self._summarize_chat_lines(self._keyscan_lines))
    ...
    # Rows in the process / telemetry / posture dialogs are (label, kind, payload):
    #   kind == "header"  → section heading, non-selectable (bold+dim)
    #   kind == "action"  → selectable; payload is the method name or audit type
    # Keep the menus split clearly:
    #   Process Investigation = selected-process actions only
    #   Live Telemetry        = event/traffic views retained for compatibility
    #   SecAuditor            = browser/API product for host-wide security
    ...
    _AUDIT_ROWS = [
        ("SecAuditor Browser/API", "header", None),
        ("Open SecAuditor browser app", "action", "open_secauditor"),
        ("Show SecAuditor commands", "action", "show_secauditor"),
    ]
    ...
    def _show_secauditor_bridge(self, launched=False, detail=""):
        lines = [
            "SecAuditor owns host-wide security auditing now.",
            "",
            "Run the browser/API product:",
            "  python3 -m secauditor serve",
            "",
            "Run one audit:",
            "  python3 -m secauditor audit auth_stack",
            "",
            "Run the full security scan:",
            "  python3 -m secauditor full-scan --html --open-html",
            "",
            "ProcMon stays focused on process monitoring, selected-process triage,",
            "and network connections for the selected process.",
        ]
        if launched:
            lines.insert(0, "SecAuditor server launch requested.")
            if detail:
            ...
            lines.extend(["", detail])
        self._audit_mode = True
        self._audit_type = "secauditor_bridge"
        self._audit_lines = lines
        self._audit_findings_structured = []
        self._audit_line_for_finding = []
        self._audit_loading = False
        self._detail_focus = True
    ...
    def _dispatch_audit_bridge(self, payload):
        """Host-wide security moved to the SecAuditor browser/API product."""
        if payload == "open_secauditor":
            try:
                subprocess.Popen(
                    [sys.executable, "-m", "secauditor", "serve"],
                    stdout=subprocess.DEVNULL,
                    stderr=subprocess.DEVNULL,
                    stdin=subprocess.DEVNULL,
                    start_new_session=True,
                )
                self._log("SECAUDITOR", "launched python3 -m secauditor serve")
                self._show_secauditor_bridge(launched=True)
            except Exception as e:
                self._log("SECAUDITOR", f"launch failed: {e}")
                self._show_secauditor_bridge(
                    launched=False,
    ...
    def _prompt_audit(self):
        """Show the SecAuditor bridge menu (a key)."""
        self._run_sectioned_menu(
            self._AUDIT_ROWS,
            title=" SecAuditor \u2014 \u2191\u2193 navigate, Enter select, Esc cancel ",
            footer="Host-wide security audits now run in the browser/API product.",
            on_select=self._dispatch_audit_bridge,
            surface="audit_menu",
        )
    ...
    def _run_loop(self, last_refresh):
        while True:
            ...
            for _scope in ("audit", "keyscan", "hidden", "inspect",
                           "events", "bulk"):
...
def main():
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "survives fork bombs and memory exhaustion. Host-wide security "
            "audits live in `python3 -m secauditor serve`."
        ),
    )
    ...
    parser.add_argument("--audit", default="",
                        help=("Legacy compatibility: run a single SecAuditor "
                              "audit headless and print results. Prefer "
                              "`python3 -m secauditor audit <name>`. Options: "
                              ...
                              "process_entitlements|injection_antidebug|"
                              "filesystem_integrity|sensitive_paths_delta|"
                              "keychain|auth_stack|binary_authorization|"
                              "tool_correlation|packages|baseline_delta|"
                              "rule_engine|global_score"))
    parser.add_argument("--automated-security-scan", "--full-audit-report",
                        dest="automated_security_scan", action="store_true",
                        help=("Legacy compatibility: run the complete "
                              "SecAuditor suite headless, combine "
                              "all available Claude/Codex/Gemini reviews into "
                              "one additive report, and print a summary-first "
                              "result. Prefer `python3 -m secauditor full-scan`."))
    parser.add_argument("--no-html-report", action="store_true",
                        help=("For --automated-security-scan, print only the "
                              "terminal report and skip the HTML artifact"))
    parser.add_argument("--no-open-report", action="store_true",
                        help=("For --automated-security-scan, write the HTML "
                              "report but do not open it in the browser"))
    ...
    if getattr(args, "automated_security_scan", False):
        # Preflight still runs so the operator sees degraded backends before
        # the scan starts, but the scan itself tolerates missing tools.
        if not _preflight(skip=args.skip_preflight, scope="all"):
            return
        report_text = _render_full_audit_report(
            progress=_headless_progress,
            html_report=not args.no_html_report,
            open_html=(not args.no_html_report and not args.no_open_report),
        )
        print(report_text)
        sys.exit(0)
